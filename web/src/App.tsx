    ...
    <div className="app-shell">
      ...
      <main className="play-area" inert={applicationModalOpen} aria-hidden={applicationModalOpen} aria-busy={launching || importing || bootstrapping}>
        <div className={`viewport-frame${showOrderCoach ? " order-coach-visible" : ""}`}>
          ...
          <p id="battlefield-help" className="visually-hidden">Pointer: tap to select or order, drag to box-select, Alt-drag or middle-drag or use two fingers to pan, trackpad scroll to pan, and pinch or Ctrl+wheel to zoom. Build completed structures from the command console, then tap a green footprint. Repair, sell, and support tools also target the battlefield. Keyboard: arrow or W A S D keys pan, Q selects, E enters contextual order mode, X stops selected units, plus and minus zoom, Home resets the camera, Escape cancels an active tool or pauses, and Space switches graphics when available. Number keys select control groups; press the same selected group again to center it. Control plus a number assigns the current selection, Shift plus a number adds a group to the selection, and Alt plus a number selects and centers it.</p>
