...
const traceTickMin = Number.parseInt(process.env.CNCWEB_VERIFY_TRACE_TICK_MIN ?? "", 10);
const traceTickMax = Number.parseInt(process.env.CNCWEB_VERIFY_TRACE_TICK_MAX ?? "", 10);
if (process.env.CNCWEB_VERIFY_TRACE_TICK_MIN !== undefined
  && (!Number.isSafeInteger(traceTickMin) || traceTickMin < 0)) {
  console.error("CNCWEB_VERIFY_TRACE_TICK_MIN must be a non-negative integer");
  process.exit(2);
}
if (process.env.CNCWEB_VERIFY_TRACE_TICK_MAX !== undefined
  && (!Number.isSafeInteger(traceTickMax) || traceTickMax < 0)) {
  console.error("CNCWEB_VERIFY_TRACE_TICK_MAX must be a non-negative integer");
  process.exit(2);
}
if (Number.isSafeInteger(traceTickMin) && Number.isSafeInteger(traceTickMax)
  && traceTickMin > traceTickMax) {
  console.error("CNCWEB_VERIFY_TRACE_TICK_MIN must be <= CNCWEB_VERIFY_TRACE_TICK_MAX");
  process.exit(2);
}
function missionEightTraceActive(tick) {
  if (!trace) return false;
  const interval = process.env.CNCWEB_VERIFY_TRACE_FINE === "1" ? 30 : 300;
  if (tick % interval !== 0) return false;
  if (Number.isSafeInteger(traceTickMin) && tick < traceTickMin) return false;
  if (Number.isSafeInteger(traceTickMax) && tick > traceTickMax) return false;
  return true;
}
const traceCompact = process.env.CNCWEB_VERIFY_TRACE_COMPACT === "1";

function missionEightTraceCompactRecord(snapshot, attackers, hostiles) {
  return {
    ...
    weap: (() => {
      const w = snapshot.objects.find((o) => (
        o.owner === HOUSE_GDI && o.type === 4 && o.typeName === "WEAP" && o.strength > 0
      ));
    ...
    // l253: hostiles near free MTNK (death-window diagnose 52200–53100).
    nearFree: (() => {
      const free = attackers.find((object) => (
        object.typeName === "MTNK"
        && object.strength > 0
        && (missionEightState.eastBPostWestProducedTankKeys.has(objectKey(object))
          || missionEightState.strikeKeys.has(objectKey(object)))
      ));
      if (!free) return [];
      // Prefer close threats; also flag south BGGY/LTNK out to d=20 for wave ETA.
      return hostiles
        .filter((h) => h.strength > 0 && (
          missionEightDistance(free, h) <= 10
          || (h.cellY >= 36 && (h.typeName === "BGGY" || h.typeName === "LTNK")
            && missionEightDistance(free, h) <= 20)
        ))
        .map((h) => ({
          typeName: h.typeName,
          id: h.id,
          ...
          cellX: h.cellX,
          cellY: h.cellY,
          d: missionEightDistance(free, h),
        }))
        .toSorted((a, b) => a.d - b.d || a.strength - b.strength)
        .slice(0, 12);
    })(),
    // l376: east pad structures for residual PROC/AFLD chip diagnosis.
    eastPad: hostiles
      .filter((h) => h.strength > 0 && h.cellX >= 38 && h.cellY <= 22 && (
        h.typeName === "AFLD" || h.typeName === "PROC" || h.typeName === "HAND"
        || h.typeName === "FACT" || h.typeName === "GUN" || h.typeName === "NUKE"
        || h.typeName === "HQ"
      ))
      .map((h) => ({
        typeName: h.typeName,
      ...
      .toSorted((a, b) => a.cellX - b.cellX || a.cellY - b.cellY)
      .slice(0, 16),
    // l387: remaining hostiles when residual (diagnose stuck ~11).
    remainH: hostiles
      .filter((h) => h.strength > 0)
      .map((h) => ({
        t: h.typeName,
    ...
    airstrike: {
      ...
      readyCount: missionEightState.airstrike.readyTicks.length,
      // l200/l212: multi-pass evidence — order count + last live target cell.
      orderCount: missionEightState.airstrike.orders.length,
      lastTarget: missionEightState.airstrike.orders.length > 0
        ? missionEightState.airstrike.orders[
          missionEightState.airstrike.orders.length - 1
        ].target
        : null,
      lastCell: missionEightState.airstrike.orders.length > 0
        ? {
          x: missionEightState.airstrike.orders[
            missionEightState.airstrike.orders.length - 1
          ].cellX,
          y: missionEightState.airstrike.orders[
            missionEightState.airstrike.orders.length - 1
          ].cellY,
        }
        : null,
      discharges: missionEightState.airstrike.discharges.length,
    },
    // l486: freeLast@83164 diagnose residual/pad latch (lost with checkout).
    residual: Boolean(missionEightState.eastBResidualCommit),
    padCleared: missionEightState.eastBEastPadClearedTick,
  };
}
...
const maxTicksOverride = Number.parseInt(process.env.CNCWEB_VERIFY_MAX_TICKS ?? "", 10);
const MAX_TICKS = Number.isSafeInteger(maxTicksOverride) && maxTicksOverride > 0
  ? Math.min(maxTicksOverride, mission.maxTicks)
  : mission.maxTicks;
if (process.env.CNCWEB_VERIFY_MAX_TICKS !== undefined
  && (!Number.isSafeInteger(maxTicksOverride) || maxTicksOverride <= 0)) {
  console.error("CNCWEB_VERIFY_MAX_TICKS must be a positive integer");
  process.exit(2);
}
...
const missionEightRoutes = {
  ...
  "east-b": [
    ...
    { cellX: 33, cellY: 12, label: "northern crossing", forceMove: true },
    {
      cellX: 43, cellY: 16, label: "southeast SAM approach", forceMove: true,
    },
    {
      cellX: 43, cellY: 14, label: "southeast SAM", typeName: "SAM",
      targetCellX: 43, targetCellY: 14,
    },
    { cellX: 50, cellY: 16, label: "eastern perimeter", forceMove: true },
    {
      cellX: 52, cellY: 14, label: "southwest SAM", typeName: "SAM",
      targetCellX: 52, targetCellY: 14,
    },
    {
      cellX: 54, cellY: 7, label: "northeast SAM approach", forceMove: true,
    },
    {
      cellX: 54, cellY: 5, label: "northeast SAM", typeName: "SAM",
      targetCellX: 54, targetCellY: 5,
    },
    { cellX: 48, cellY: 8, label: "eastern base", forceMove: true },
  ],
};
...
// v539: south-of-pad approach so the dive does not path straight through the
// LTNK pair at 25,12–14; also the re-kite hold after AFLD falls.
...
function eastAProductionGunHoldTimeout(state) {
  return eastAEarlyCapture(state) ? 1_200 : 10_500;
}

// Early-capture stage-7 parks on the southwest SAM line (safe from the
// production turret). Stage-8 pre-assault waits at western production staging
// — closer ridge cells ({18,22}/{23,20}) wiped the wave before HAND took
// damage (v423). Best prior class held farther back and left HAND at ~104.
function eastAProductionGunHoldCell(state) {
  return eastAEarlyCapture(state) ? { cellX: 9, cellY: 22 } : missionEightEastAProductionStaging;
}

function eastAHandWaitHoldCell(state) {
  return eastAEarlyCapture(state)
    ? missionEightEastAProductionStaging
    : eastAProductionGunHoldCell(state);
}

function eastAEarlyCapture(state) {
...
function eastAShouldOpenHandAssault(state, snapshot, hand, ticksSinceAir) {
  ...
  if (eastAEarlyCapture(state)) {
    return ticksSinceAir >= 6_800
      || state.airstrike.pending !== undefined
      || (hand !== undefined && hand.strength <= 200);
  }
  return ticksSinceAir >= 7_100
    || state.airstrike.pending !== undefined
...
function eastAPrepareEarlyCaptureCleanup(state, hostiles, snapshot) {
  ...
  while (state.westCleanupStage < 7) {
    const site = missionEightEastAWestCleanupTargets[state.westCleanupStage];
    ...
    state.westCleanupProgress.push({
      tick: snapshot.tick,
      stage: state.westCleanupStage,
      label: site.label,
      typeName: site.typeName,
      cellX: site.cellX,
      cellY: site.cellY,
      absentAtStart: target === undefined,
      skippedForEarlyCapture: true,
    });
    state.westCleanupStage += 1;
  }
  state.westCleanupDeferredFromStage ??= 3;
  state.productionGunHoldStartedTick ??= snapshot.tick;
}

const missionEightEastAPostFactCleanupTransitRoute = [
...
function eastAEngineerWesternGun(hostiles) {
  return hostiles.find((hostile) => (
    hostile.typeName === "GUN"
    && hostile.cellX === missionEightEastAEngineerWesternGunCell.cellX
    && hostile.cellY === missionEightEastAEngineerWesternGunCell.cellY
  ));
}
...
const missionEightState = {
  ...
  // Free tanks that emerged after western SAM death — remaining-SAM rebuild cohort.
  eastBPostWestProducedTankKeys: new Set(),
  // First post-west free emergence tick (timed SAM release if picket never quiets).
  eastBPostWestFreeEmergeTick: undefined,
  // Pinned WEAP MTNK key until free emerges healthy (TRACE baseline atk=0 @35.4k).
  ...
  eastBSamSpineFinisherKey: undefined,
  eastBSamSpineFinisherHoldKeys: new Set(),
  ...
  eastBPostAllSamWeapSellTick: undefined,
  // l249d: free first reached village ridge; delayed spare sell / PYLE rebuild.
  eastBFreeRidgeTick: undefined,
  // l294c: sticky BGGY focus id while free trades wave (avoid #6/#8 thrash).
  eastBBggyFocusKey: undefined,
  // l331: latch east AFLD sighted so residual can treat true death (not fog).
  eastBSawEastAfld: false,
  // l346: latch residual commit so free does not bounce back to village bait.
  eastBResidualCommit: false,
  // l519u: free left pad for hospital peel — do not re-soft-hold.
  eastBFreeHospPeel: false,
  // l519cr: free late door hunt latch (launch from softHold@≥56k free≥160).
  eastBLateDoorHunt: false,
  // l519ev: latch north residual so door wave does not snap free off west nest.
  eastBNorthResidual: false,
  ...
  // Post-western-SAM rebuild: set when ≥3 healthy free/strike MTNKs exist.
  eastBPostWestRebuildReadyTick: undefined,
...
function observeMissionEightTurn(snapshot, friendly, hostiles) {
  ...
  if (mission.variant === "east-b") {
    ...
    for (const tank of liveTanks) {
      ...
      if (eastBWesternSamDead(hostiles) && state.allSamsDeadTick === undefined) {
        state.eastBPostWestProducedTankKeys.add(key);
        state.eastBPostWestFreeEmergeTick ??= snapshot.tick;
      }
      if (state.assaultTick !== undefined
        && state.eastBSamSpineFinisherKey === undefined
        && tank.cellY >= 28) {
        state.eastBSamSpineFinisherKey = key;
        state.eastBSamSpineFinisherHoldKeys.add(key);
      ...
      // Post-western-SAM: first free tank → strike (remaining SAMs). TRACE v395b
      // auto-base left bg=1 strike=0 forever while NW SAM stayed 400. Only staff
      // WEAP when factory is hurt/threatened AND we already have a strike tank,
      // or when a 2nd free emerges. Village is surplus after strike.
      if (eastBWesternSamDead(hostiles)
        ...
        const liveStrikeMtnk = liveTanks.filter((unit) => (
          unit.typeName === "MTNK" && unit.strength > 0
        ...
        const weapCell = weaponFactory
          ? { cellX: weaponFactory.cellX, cellY: weaponFactory.cellY }
          : { cellX: 36, cellY: 55 };
        const weapHurt = Boolean(weaponFactory
          && weaponFactory.strength < Math.ceil(weaponFactory.maxStrength * 0.75));
        const weapThreatNear = hostiles.some((hostile) => (
          (hostile.type === 1 || hostile.type === 2)
          && missionEightDistance(hostile, weapCell) <= 10
        ));
        if (weaponFactory && liveBaseMtnk < 1 && liveStrikeMtnk >= 1) {
          // 2nd free: WEAP picket first (TRACE v395g WEAP died with vg=2 bg=0).
          clearMissionEightUnitRoleKey(key);
          state.baseGuardKeys.add(key);
          state.strikeKeys.delete(key);
          state.villageGuardKeys.delete(key);
        } else if (liveStrikeMtnk >= 1 && liveVillageMtnk < 1
          && (weapThreatNear || weapHurt || liveBaseMtnk >= 1)) {
          clearMissionEightUnitRoleKey(key);
          state.villageGuardKeys.add(key);
          state.strikeKeys.delete(key);
          state.baseGuardKeys.delete(key);
        } else {
          // Sole free (or first free) — go kill remaining SAMs.
          state.strikeKeys.add(key);
          state.villageGuardKeys.delete(key);
          state.baseGuardKeys.delete(key);
        }
...
function queueMissionEightRepairs(snapshot, friendly, commands) {
  const state = missionEightState;
  // Pre-assault east-b: skip routine repairs so cash banks for free tanks.
  // Post-assault / post-western-SAM: always repair WEAP/PROC/NUKE/PYLE — TRACE
  // v391k lost WEAP after the kill (producedTanks stuck at 4, funds@1340 idle).
  const eastBProducedCount = friendly.filter((object) => (
    object.typeName === "MTNK"
    && state.eastBProducedTankKeys.has(objectKey(object))
  )).length;
  const eastBCriticalRepair = mission.variant === "east-b"
    && (state.assaultTick !== undefined || eastBWesternSamDead());
  if (mission.variant === "east-b" && eastBProducedCount < 6 && !eastBCriticalRepair) return;
  ...
  // Post-assault east-b: WEAP+PROC only until a free rebuild tank exists.
  // TRACE v392c: WEAP held @400 through 37k but funds 740→354 on NUKE/PYLE
  // repairs — never reached 800 for the next MTNK.
  const weapBuilding = friendly.find((object) => (
    object.type === 4 && object.typeName === "WEAP" && object.strength > 0
  ));
  const weapNeedsRepair = Boolean(
    weapBuilding && weapBuilding.strength < weapBuilding.maxStrength
  );
  const freeRebuildTanks = friendly.filter((object) => (
    object.typeName === "MTNK"
    && state.eastBProducedTankKeys.has(objectKey(object))
    && !state.villageGuardKeys.has(objectKey(object))
    && object.strength > 0
  )).length;
  const bankForRebuildTank = eastBCriticalRepair
    && eastBWesternSamDead()
  ...
  const postWestWeapRepair = eastBCriticalRepair && eastBWesternSamDead();
  const repairCadence = postWestWeapRepair ? 60 : (eastBCriticalRepair ? 180 : 900);
  for (const building of friendly.filter((object) => (
    object.type === 4 && object.strength < object.maxStrength
    && !(object.objectFlags & (1 << 1))
    && (mission.variant === "east-a" ? importantEastA.has(object.typeName)
      : mission.variant === "east-b"
        ? (eastBCriticalRepair
          ? importantEastB.has(object.typeName)
          : true)
        : true)
    // Hold only cosmetic WEAP repair while banking MTNK. Always repair when
    // WEAP drops below 95% post-west (v395n) or 75% otherwise.
    && !(bankForRebuildTank
      && object.typeName === "WEAP"
      && object.strength >= Math.ceil(object.maxStrength
        * (postWestWeapRepair ? 0.95 : 0.75))
      && funds < 900)
    && funds >= (postWestWeapRepair && object.typeName === "WEAP" ? 100 : 200)
    && snapshot.tick - (state.repairTicks.get(object.id) ?? -repairCadence) >= repairCadence
  )).toSorted((left, right) => {
    // WEAP first — without it the post-west rebuild cannot produce tanks.
    const rank = (name) => (name === "WEAP" ? 0 : name === "PROC" ? 1
      : name === "NUKE" ? 2 : name === "PYLE" ? 3 : 4);
    return rank(left.typeName) - rank(right.typeName)
      || left.strength - right.strength
      || left.id - right.id;
  })) {
    commands.push({ type: COMMAND_STRUCTURE, args: [STRUCTURE_REPAIR_START, 0, 0, 0, 0, 0, 0] });
    ...
    repairOrders += 1;
    // One repair order per tick is enough — cash is tight post-west.
    if (eastBCriticalRepair) break;
  }
}
...
function queueMissionEightBase(snapshot, friendly, hostiles, commands) {
  ...
  if (mission.variant === "east-b"
    ...
    // (l66 FACT 800→255→0). l249j keep-FACT broke free path (funds stuck 740,
    // free never clears SAMs). Must sell for freeT/GUN.
    // l519e4/e5 second-NUKE-before-FACT CLOSED (maxNukes=1 or free mid death).
    ...
    // After free is out, sell PROC/PYLE for partner free (l252i load-bearing).
    // l519dd/dr/dw/dx keep-PYLE cascades CLOSED (freeT=2 mid death / deadlock).
    // PYLE sell required for freeT=3 mid-SAM (da free@170@60138).
    if (freeBand.length === 1 && liveFundsNow < 800) {
      const villageInfNow = friendly.filter((object) => (
        object.type === 1 && object.strength > 0
        && state.villageGuardKeys.has(objectKey(object))
      )).length;
      const delaySellForVillageScreen = builtAssets.has("PYLE")
        && villageInfNow < 4
        && liveFundsNow >= 900;
      if (!delaySellForVillageScreen) {
        if (state.eastBPostWestProcSellTick === undefined) {
          ...
          || state.eastBPostWestPyleSellTick < 0) {
          if (state.eastBPostWestPyleSellTick !== undefined
            && state.eastBPostWestPyleSellTick < 0) {
            state.eastBPostWestPyleSellTick = undefined;
          }
          const pyle = buildings.find((object) => (
  ...
  if (mission.variant === "east-b" && !builtAssets.has("FACT")) {
    ...
    // l518: residual free@pad WEAP sell for village — no FACT and no WEAP left;
    // still must reach PYLE rebuild + E1 buys (l518b funds=1090 but early-return
    // starved PYLE → civ@44k).
    const residualVillageRestaffOpen = Boolean(state.eastBResidualCommit)
      || state.eastBPostAllSamWeapSellTick !== undefined;
    if (!(eastBWesternSamDead()
      && (builtAssets.has("WEAP") || residualVillageRestaffOpen))) {
      return;
    }
  }
  ...
  if (mission.variant === "east-b") {
    ...
    // l518: once residual free@pad, allow PYLE rebuild for village restaff
    // (l512/l517: funds=90, no PYLE, vg→0, civ-nine@50k; l479 held n=8 to 83k).
    const residualNeedsPyle = Boolean(state.eastBResidualCommit)
      && missingAsset === "PYLE";
    const skipSoldSupportRebuild = postWestBankingTank
      ...
      && !residualNeedsPyle;
    // After residual WEAP sell, do not rebuy PROC/WEAP — refund is for PYLE+E1.
    const skipAfterVillageWeapSell = state.eastBPostAllSamWeapSellTick !== undefined
      && (missingAsset === "PROC" || missingAsset === "WEAP");
    if (missingAsset && !skipNukeRebuildForTank && !skipSoldSupportRebuild
      && !skipAfterVillageWeapSell) {
  ...
  const freeEastBTanks = mission.variant === "east-b"
    ...
    : 0;
  // Post-west rebuild cohort: only tanks that emerged after western SAM death.
  // Pre-kill free tanks must not stop MTNK banking (TRACE v395c funds stuck 740).
  const freeEastBPostWestTanks = mission.variant === "east-b"
    ? friendly.filter((object) => (
      object.typeName === "MTNK"
      && state.eastBPostWestProducedTankKeys.has(objectKey(object))
      && !state.villageGuardKeys.has(objectKey(object))
  ...
  if (mission.variant === "east-b" && builtAssets.has("WEAP")) {
    // Always prefer MTNK once WEAP is up — western SAM / GUN need continuous
    // armor (freeT=3 2v1). After all-SAM + free on ridge, skip MTNK spam ONLY
    // when we already have ≥2 free post-west tanks (l377 sole free dies@pad;
    // need residual partner built during village hold).
    const freeOnRidgeForMtnk = state.allSamsDeadTick !== undefined
      && friendly.some((object) => (
        object.typeName === "MTNK"
        && state.eastBPostWestProducedTankKeys.has(objectKey(object))
        && object.strength >= 100
        && object.cellY >= 46 && object.cellX <= 16
      ));
    // l410: count ANY free MTNK after all-SAM (post-west keys miss tanks that
    // emerge after allSamsDeadTick is set). Sole residual free dies@pad; need
    // free#2 from cash. funds stuck 90 from ~36k — multi-sell spare structures.
    const freeStagingLive = freeEastBTanks;
    // l513: free@pad healthy holds alone (l512 free@158) — don't bank partner
    // MTNK over village E1 (funds stuck 90, civ-nine@50k, no village screen).
    const freeOnPadHealthy = friendly.some((object) => (
      object.typeName === "MTNK"
      && object.strength >= 100
      && object.cellX >= 36 && object.cellY <= 16
      && (state.eastBPostWestProducedTankKeys.has(objectKey(object))
        || state.strikeKeys.has(objectKey(object)))
    ));
    // l519e4 dual-NUKE partner CLOSED (second NUKE never places before FACT
    // sell; keep-PROC free mid death@42k). free@pad suppresses partner (da).
    const needResidualPartner = state.allSamsDeadTick !== undefined
      && freeStagingLive < 2
      && !freeOnPadHealthy;
    const anyFreeOnRidge = state.allSamsDeadTick !== undefined
      && friendly.some((object) => (
        object.typeName === "MTNK"
        && object.strength >= 80
        && object.cellY >= 44 && object.cellX <= 18
      ));
    const residualWantsPartner = needResidualPartner
      && (freeOnRidgeForMtnk || anyFreeOnRidge || state.eastBResidualCommit
        || snapshot.tick >= state.allSamsDeadTick + 300);
    const partnerSellCount = state.eastBResidualPartnerSellCount ?? 0;
    const partnerLastSell = state.eastBResidualPartnerSellTick ?? -Infinity;
    const liveCash = snapshot.sidebar.credits + snapshot.sidebar.tiberium;
    // l513: funds stuck ~90 after free@pad (l512/l513) — sell spare for village
    // E1 when residual free holds and village thin (civ-nine@50k before pass3).
    const villageLiveCount = friendly.filter((u) => (
      u.type === 1 && u.strength > 0 && state.villageGuardKeys.has(objectKey(u))
    )).length;
    const wantVillageCash = Boolean(state.eastBResidualCommit)
      && freeStagingLive >= 1
      && villageLiveCount < 6
      && liveCash < 250;
    if ((residualWantsPartner && liveCash < 800 || wantVillageCash)
      && partnerSellCount < 12
      && snapshot.tick >= partnerLastSell + 90) {
      const nukeN = friendly.filter((o) => (
        o.type === 4 && o.typeName === "NUKE" && o.strength > 0
        && !state.soldStructureIds.has(o.id)
      )).length;
      // Never sell WEAP/PROC/FACT/HQ/PYLE/HOSP (radar/A-10/prod/income).
      // Residual WEAP sell is a separate path below (only free@pad healthy).
      const keep = new Set(["WEAP", "PROC", "FACT", "HQ", "PYLE", "HOSP", "BIO"]);
      const sellables = friendly.filter((o) => (
        o.type === 4
        && o.strength > 0
        && !state.soldStructureIds.has(o.id)
        && !keep.has(o.typeName)
        && (o.typeName !== "NUKE" || nukeN >= 2)
      )).toSorted((a, b) => {
        const rank = (u) => (
          u.typeName === "SILO" ? 0
            : u.typeName === "GTWR" ? 1
        ...
        return rank(a) - rank(b) || a.strength - b.strength || a.id - b.id;
      });
      if (sellables[0]) {
        sellMissionSevenStructure(commands, sellables[0]);
        state.soldStructureIds.add(sellables[0].id);
        state.eastBResidualPartnerSellTick = snapshot.tick;
        state.eastBResidualPartnerSellCount = partnerSellCount + 1;
        state.saleOrders.push({
          tick: snapshot.tick,
          typeName: sellables[0].typeName,
          cellX: sellables[0].cellX,
          cellY: sellables[0].cellY,
          reason: residualWantsPartner
            ? "east-b residual partner free funding"
            : "east-b residual village E1 funding",
        });
      }
    }
    ...
    // l518i: no WEAP/PYLE village sell experiments — free path is load-bearing
    // (l518d–h keep-PYLE freeLast@42k; WEAP without PYLE cash unspendable).
    // free@pad healthy still suppresses needResidualPartner above.
    // l519e1–e5 partner/dual-NUKE CLOSED.
    if (eastBMtnkEntry && !eastBMtnkEntry.constructing && !eastBMtnkEntry.completed
      ...
      && (needResidualPartner || !freeOnRidgeForMtnk || freeStagingLive < 2)) {
    ...
    // Post-western-SAM (v391): TRACE v390/v391c sat at funds≈740 with no 5th
    // tank while civs died. Sell spare NUKE once to fund village + rebuild armor.
    const villageTanksForSale = friendly.filter((object) => (
      object.typeName === "MTNK"
      && state.villageGuardKeys.has(objectKey(object))
      && object.strength > 0
    )).length;
    const westernSamDeadForSale = eastBWesternSamDead(hostiles);
    // Fund 1st free rebuild, or 2nd free (picket) while first free holds WEAP.
    // TRACE v396 free=1 funds~0 after spawn — never produced a partner tank.
    const postWestNeedsCash = westernSamDeadForSale
      && state.allSamsDeadTick === undefined
      && (state.eastBPostWestRebuildReadyTick === undefined
    ...
    if (postWestNeedsCash
      ...
      if (sellables.length > 0) {
        ...
        state.saleOrders.push({
          ...
          cellX: sell.cellX,
          cellY: sell.cellY,
          reason: freeNeedsPartner
            ? "east-b post-west partner free tank funding"
            : "east-b post-west rebuild tank funding",
        });
      }
    }
    // l250: no WEAP sell while PYLE/FACT sold — cash unspendable.
    // l519da: WEAP sell@56k free pure softHold y≤12 str≥150 → lose@60138.
    // l519e1 WEAP@50k for partner CLOSED (factory gone before MTNK).
    // l519de earlier@54.5k y≤16 regressed lose@58996 — keep da timing.
    const freePureSoftHold = friendly.some((u) => (
      u.typeName === "MTNK" && u.strength >= 150
      && u.cellX >= 36 && u.cellY <= 12
      && (state.eastBPostWestProducedTankKeys.has(objectKey(u))
        || state.strikeKeys.has(objectKey(u)))
    ));
    if (Boolean(state.eastBResidualCommit)
      && freePureSoftHold
      && snapshot.tick >= 56_000
      && state.eastBPostAllSamWeapSellTick === undefined
      && builtAssets.has("WEAP")) {
      const weapSell = friendly.find((o) => (
        o.type === 4 && o.typeName === "WEAP" && o.strength > 0
        && !state.soldStructureIds.has(o.id)
      ));
      if (weapSell) {
        sellMissionSevenStructure(commands, weapSell);
        state.soldStructureIds.add(weapSell.id);
        state.eastBPostAllSamWeapSellTick = snapshot.tick;
        state.saleOrders.push({
          tick: snapshot.tick,
          typeName: weapSell.typeName,
          cellX: weapSell.cellX,
          cellY: weapSell.cellY,
          reason: "east-b residual softHold WEAP sell (l519da best@60138)",
        });
      }
    }
    ...
    // l519e4/e5 dual-NUKE door partner CLOSED (never places / free mid death).
    const eastBNukeCount = buildings.filter((object) => (
      object.typeName === "NUKE" && object.strength > 0
    )).length;
    const eastBBankingRebuildMtnk = eastBWesternSamDead(hostiles)
      && freeEastBPostWestTanks < eastBPostWestRebuildTankCount
      && state.eastBPostWestRebuildReadyTick === undefined
      && funds < 800;
  ...
  // Post-all-SAM: always allow production when PYLE up (village E1 refill).
  const canProduceEastB = mission.variant === "east-b" && builtAssets.has("PYLE")
  ...
  if (canProduceEastA || canProduceEastB) {
    ...
    const eastBRebuildWave = mission.variant === "east-b"
      ...
      && !eastBSamChipped;
    // Post-western-SAM rebuild: bank MTNK until the healthy free/strike cohort
    // is ready for remaining SAMs (TRACE v390 produced only 4 tanks total).
    // l178: after all SAMs also bank MTNK for eastern base assault (free dies
    // trading AFLD alone; need 2–3 free on standoff).
    // l519ag: free@pad healthy + village restaff — do not bank 800 for free#2–3
    // (WEAP-sell refund must hit PYLE+E1 before civ wall@47k).
    const freePadVillageRestaff = mission.variant === "east-b"
      ...
      && (!builtAssets.has("PYLE")
        || eastBVillageInfantryCount < 8
        || state.eastBPostAllSamWeapSellTick !== undefined);
    const eastBPostSamAssaultBank = mission.variant === "east-b"
      && missionEightState.allSamsDeadTick !== undefined
      && freeEastBPostWestTanks < 3
      && !eastBMtnkBusy
      && funds < 800
      && !freePadVillageRestaff;
    const eastBPostWestRebuild = mission.variant === "east-b"
      && missionEightState.assaultTick !== undefined
      && !eastBSamStillUp
      && missionEightState.allSamsDeadTick === undefined
      && missionEightState.eastBPostWestRebuildReadyTick === undefined
      && !eastBMtnkBusy;
    ...
    const eastBFinishBank = mission.variant === "east-b"
      ...
      && (eastBSamMtnkBank
        || (eastBSamChipped
          && freeEastBTanks < 1
          && !eastBMtnkBusy)
        || eastBPostWestRebuild
        || eastBPostSamAssaultBank);
    ...
    // While banking for the next tank, still allow E3 if we cannot afford MTNK
    // this tick (funds in [300,799]) so rockets stockpile beside staged tanks.
    // TRACE l15: free@400 at y=42 when civ lose ends game — village E3 after
    // free is building (mtnk busy) or free already out, never before MTNK buy.
    // TRACE l16–l22: E3 drained funds to ~9 before partner free could start;
    // bank hard for 2nd free while freePostWest < 2 and WEAP still lives.
    // TRACE l16–l22: E3 drained funds before partner free; bank hard for 2nd
    // free while freePostWest < 2 and WEAP still lives.
    const eastBPartnerFreeBank = mission.variant === "east-b"
      && eastBWesternSamDead(hostiles)
      && state.allSamsDeadTick === undefined
      && freeEastBPostWestTanks >= 1
      && freeEastBPostWestTanks < 2
      && !eastBMtnkBusy
      && funds < 800;
    // l513: residual free@pad funds~90 blocked village E1 (needed funds>=200).
    // Allow residual village buy when funds >= E1 cost (~100).
    const eastBPostWestVillageInf = mission.variant === "east-b"
      && eastBWesternSamDead(hostiles)
      && (eastBMtnkBusy || freeEastBPostWestTanks >= 1
        || state.allSamsDeadTick !== undefined
        || Boolean(state.eastBResidualCommit))
      && !eastBPartnerFreeBank
      && (funds >= 200
        || (Boolean(state.eastBResidualCommit) && funds >= 100));
    const eastBInfantryReserve = mission.variant === "east-b" && (
      eastBBankingForArmor || eastBPartnerFreeBank
    ) && !eastBPostWestVillageInf
      ? 800
    ...
    const canBuyPackRocket = mission.variant === "east-b" && eastBSamPackPhase
      ...
      && (eastBMtnkBusy || funds < 800 || freeEastBTanks >= missionEightEastBAssaultTankCount
        || eastBPostWestVillageInf);
    const villageInfCount = friendly.filter((u) => (
      u.type === 1 && u.strength > 0 && state.villageGuardKeys.has(objectKey(u))
    )).length;
    // l206: SE SAM window (post-west, free≥1) still loses civs 6–8 @37–39k then
    // 9th@40.5k. Cap was 2 while banking partner free — raise to 5 once first
    // free is out so village screen staffs before allSams. Post-all-SAM: 8.
    // l241: raise post-all-SAM village E1 cap (was 8) so HARV income after
    // keeping PROC staffs screen through A-10 pass-3 (~54300).
    // l516: cap 10 post-west spent free cash (freeT0 freeLast@36900). Restore
    // cap 5 once free out; 16 only after residual/allSams with free@pad.
    // l519aq–ar pre-sell E1 cap/reserve CLOSED (free mid maxAir=0).
    const villageInfCap = (state.allSamsDeadTick !== undefined
      || Boolean(state.eastBResidualCommit))
      ? 16
      : freeEastBPostWestTanks >= 1 && eastBWesternSamDead(hostiles)
        ? 5
        : freeEastBPostWestTanks < 2
          && eastBWesternSamDead(hostiles)
          ? 3
          : 4;
    // Prefer E1 for village screen (cheap pad holds) when free≥1 post-west.
    const villageInfEntry = mission.variant === "east-b"
      && freeEastBPostWestTanks >= 1
      && eastBWesternSamDead(hostiles)
      ? (snapshot.sidebar.entries.find((entry) => entry.assetName === "E1")
        ?? infantry)
      : infantry;
    // l254d: burst village E1 while PYLE still up (delay PROC/PYLE sell until
    // villageInf≥4). Keep ≥800 bank when partner free still outstanding so MTNK
    // path does not regress (l254 keep-PYLE free died@37500).
    // l515: once residual latched free@pad, don't bank 800 for partner MTNK —
    // spend on village E1 (funds→90 forever otherwise).
    const partnerStillNeeded = freeEastBPostWestTanks < 2
      && state.allSamsDeadTick === undefined
      && !Boolean(state.eastBResidualCommit);
    const villageBurstReserve = partnerStillNeeded ? 800 : 0;
    const canBuyVillageInf = eastBPostWestVillageInf
      && villageInfEntry && !villageInfEntry.constructing && !villageInfEntry.completed
      && !villageInfEntry.onHold && !villageInfEntry.busy
      && funds >= villageInfEntry.cost + villageBurstReserve
      && villageInfCount < villageInfCap;
    if (canBuyVillageInf) {
      startMissionEightProduction(commands, villageInfEntry);
      funds -= villageInfEntry.cost;
    } else if (canBuyInfantry || canBuyPackRocket) {
      startMissionEightProduction(commands, infantry);
      funds -= infantry.cost;
    }
  }
  ...
  // l196/l200: east-b A-10 is ordered from post-west all-SAM path (AFLD-first
  // multi-pass). Suppress queueMissionEightBase A-10 for east-b so it cannot
  // waste the first charge on west HAND@5,12 (l199 HAND@800, AFLD@1000).
  const holdingEastBFreeAfld = mission.variant === "east-b"
    && state.allSamsDeadTick !== undefined;
  if (airstrikeEntry?.completed && !state.airstrike.pending && hostiles.length > 0
    && !holdingEastACounterattackStrike
    && !holdingEastBFreeAfld) {
    ...
    const counterattackSite = (state.postSamCounterattackLaunchTick !== undefined
      ...
      && hostiles.some((hostile) => (
        ...
        && hostile.cellX === missionEightEastAEngineerWesternGunCell.cellX
        && hostile.cellY === missionEightEastAEngineerWesternGunCell.cellY
      )) ? { ...missionEightEastAEngineerWesternGunCell, typeName: "GUN" } : undefined;
    ...
    const engineerTransport = engineerIngressPending && state.engineer.transportKey !== undefined
      ? friendly.find((object) => objectKey(object) === state.engineer.transportKey)
      : undefined;
    ...
    const lastWesternGunStrike = engineerAtBasinHold
      ? state.airstrike.orders.findLast((order) => (
        order.target === "GUN"
        && order.cellX === missionEightEastAEngineerWesternGunCell.cellX
        && order.cellY === missionEightEastAEngineerWesternGunCell.cellY
      ))
      : undefined;
    ...
    const productionGunPrelaunchTarget = mission.variant === "east-a"
      ...
      ? productionGun
      : undefined;
    const lastProductionGunHoldStrike = productionGun
      && state.postFactCleanupLaunchTick !== undefined
      ...
      ? state.airstrike.orders.findLast((order) => (
        ...
        && order.cellX === missionEightEastAProductionGunSite.cellX
        && order.cellY === missionEightEastAProductionGunSite.cellY
        && order.tick >= state.postFactCleanupLaunchTick
      ))
      : undefined;
    const productionGunHoldStrikeDue = productionGun
      && state.postFactCleanupLaunchTick !== undefined
      && state.westCleanupStage === 7
      && productionGun.strength > missionEightEastAProductionGunSoftStrength
      && (!lastProductionGunHoldStrike
        || snapshot.tick >= lastProductionGunHoldStrike.tick
          + (eastAEarlyCapture(state) ? 300 : 360))
      ? productionGun
      : undefined;
    // Once the production turret is soft (or HAND peel) is committed, prefer
    // A-10s on HAND/AFLD over the turret. Charging while the GUN is still
    // healthy disabled hold-strikes and sent A-10s to HAND too early.
    const productionGunAssaultReady = !productionGun
      || productionGun.strength <= missionEightEastAProductionGunSoftStrength;
    const peelCommitted = state.productionGunPeelKeys.size > 0
      || state.westCleanupStage >= 8
      || (state.productionGunChargeTick !== undefined && productionGunAssaultReady);
    ...
    // l200: east-b A-10 exclusive to post-west all-SAM path (AFLD-first multi-pass).
    const eastBAirstrikeTarget = undefined;
    const target = engineerUnloadPathArtyStrikeTarget
      ?? productionGunHoldStrikeDue
      ?? productionGunPrelaunchTarget
      ?? westernGunStrikeTarget
      ?? eastBAirstrikeTarget
      ?? ((peelCommitted
    ...
    if (state.engineer.transportRouteStage === finalTransportStage
      ...
      if (captureRush) {
        ...
        const effectiveStage = gunBlocksBasin
          ? Math.min(state.postSamCounterattackStage, 2)
          : state.postSamCounterattackStage;
        const routeTarget = missionEightEastAPostSamCounterattackRoute[effectiveStage];
...
function missionEightAssignRoles(snapshot, attackers, hostiles = []) {
  ...
  if (mission.variant === "east-b") {
    ...
    // Post-western-SAM (v391): re-fill village first — TRACE v390 loaned every
    // tank into the kill then left hospital naked (civ deaths 34.5k+).
    ...
    const westernSamDeadForAssign = eastBWesternSamDead(hostiles);
    const samNeedsFinish = Boolean(
      state.assaultTick !== undefined
      && westernSamForAssign
      && westernSamForAssign.strength < westernSamForAssign.maxStrength
    );
    // After western SAM dies, hard-prefer village refill over NW solo push until
    // we have 2 village tanks again (civ-near-threshold lose window).
    const villageProtectAfterWest = Boolean(
      westernSamDeadForAssign
      && state.allSamsDeadTick === undefined
    );
    for (const attacker of unassigned) {
      ...
      // Post-west: also never dump free MTNKs into village before remaining SAMs
      // (TRACE v395b villageProtect stole free armor; NW SAM stayed 400).
      if (attacker.typeName === "MTNK"
      ...
      // Post-west / post-all-SAM: infantry pads village. l235 free holds@11,48
      // but civ-9@49640 — raise post-allSAM village infantry cap.
      if ((villageProtectAfterWest || state.allSamsDeadTick !== undefined)
        && attacker.type === 1
        && state.villageGuardKeys.size < (state.allSamsDeadTick !== undefined ? 24 : 12)) {
        state.villageGuardKeys.add(key);
        continue;
      }
      if (state.assaultTick !== undefined) {
        if (attacker.typeName === "MTNK" || attacker.type === 2
          ...
          // Post-west MTNK: strike first. Village only from surplus (handled in
          // post-west home defense / promote), never here.
          if (villageProtectAfterWest && attacker.typeName === "MTNK") {
            state.strikeKeys.add(key);
            state.villageGuardKeys.delete(key);
            state.baseGuardKeys.delete(key);
            continue;
  ...
  if (state.assaultTick === undefined && assaultReady) {
    ...
    if (mission.variant === "east-a") {
      ...
      for (const attacker of attackers) {
        ...
        // Keep one base MTNK home when we already have ≥3 free produced tanks
        // (TRACE v392: base stripped → WEAP dies mid-kill → producedTanks stuck
        // at 4). Free wave still launches at 3; only the starting base scrap stays.
        if (state.baseGuardKeys.has(key) && attacker.typeName === "MTNK") {
          ...
          const baseMtnkStillHome = attackers.filter((unit) => (
            unit.typeName === "MTNK" && state.baseGuardKeys.has(objectKey(unit))
            && unit.strength > 0
            && !state.strikeKeys.has(objectKey(unit))
          )).length;
          if (freeTanks >= 3 && baseMtnkStillHome <= 1) continue;
  ...
  if (mission.variant === "east-b" && state.assaultTick !== undefined) {
    ...
    const westernSamDead = eastBWesternSamDead(hostiles);
    // friendly list not available here — WEAP gate applied in post-west push.
    // After western SAM dies, do not empty the hospital for a thin NW push
    // (TRACE v390: villageGuard 0 by 34.2k → civ deaths 34.5k+).
    const villageProtectAfterWest = Boolean(
      westernSamDead
      && state.allSamsDeadTick === undefined
      && liveVillageTanks < villageTankTarget
    ...
    const joiners = attackers.filter((attacker) => {
      ...
      if (attacker.typeName === "MTNK") {
        ...
        if (state.baseGuardKeys.has(key)) {
          ...
          // Never empty the last base MTNK while free produced tanks still live —
          // WEAP needs a permanent picket (TRACE v391k baseGuard MTNK=0 by 35.1k).
          const baseMtnkLive = attackers.filter((unit) => (
            unit.typeName === "MTNK" && state.baseGuardKeys.has(objectKey(unit))
            && unit.strength > 0
          )).length;
          const freeProducedLive = attackers.filter((unit) => (
            unit.typeName === "MTNK"
            && state.eastBProducedTankKeys.has(objectKey(unit))
            && !state.villageGuardKeys.has(objectKey(unit))
            && unit.strength > 0
          )).length;
          if (baseMtnkLive <= 1 && freeProducedLive >= 2) return false;
          // After western SAM dies, last base tank stays home for WEAP.
          if (westernSamDead && baseMtnkLive <= 1) return false;
          // samFinishUrgent may still pull surplus base tanks, never the last one
          // when free produced armor is already on the corridor.
          if (samFinishUrgent && attacker.strength >= 20 && baseMtnkLive > 1) {
            return true;
          }
    ...
    if (westernSam
      ...
      && westernSam.strength < westernSam.maxStrength) {
      for (const tank of attackers) {
        if (tank.typeName !== "MTNK" || tank.strength < 100) continue;
        const key = objectKey(tank);
        if (state.eastBSamSpineFinisherHoldKeys.has(key)) continue;
        if (!state.eastBWaveTwoKeys.has(key)) continue;
        if (tank.cellY > 48 || tank.cellX < 10 || tank.cellX > 15) continue;
        state.strikeKeys.add(key);
        state.eastBWaveTwoKeys.delete(key);
      }
      for (const tank of attackers) {
    ...
    if (samKillWindow) {
      if (westernSam && westernSam.strength <= 240) {
        for (const tank of attackers) {
          if (tank.typeName !== "MTNK" || tank.strength < 80) continue;
          const key = objectKey(tank);
          if (!state.eastBWaveTwoKeys.has(key)) continue;
          if (tank.cellY > 48 || tank.cellX < 10 || tank.cellX > 15) continue;
          state.strikeKeys.add(key);
    ...
    // Post-western-SAM home defense until all SAMs dead.
    // TRACE baseline: atk=0 by 35.4k while free builds → free spawns @262 into
    // pad fire. Pin ≥1 WEAP defender (non-postWest preferred) until free emerges
    // and is past the factory band; no village MTNK loans until free exists.
    if (westernSamDead && state.allSamsDeadTick === undefined) {
      const rebuildReady = state.eastBPostWestRebuildReadyTick !== undefined;
      const freePostWestLive = attackers.filter((unit) => (
        unit.typeName === "MTNK"
        && state.eastBPostWestProducedTankKeys.has(objectKey(unit))
        && unit.strength >= 40
      ));
      const freePostWestCount = freePostWestLive.length;
      // Drop dead defender pin.
      if (state.eastBPostWestWeapDefenderKey) {
        const still = attackers.find((u) => (
          objectKey(u) === state.eastBPostWestWeapDefenderKey && u.strength > 0
        ));
      ...
      if (needPinnedDefender && !state.eastBPostWestWeapDefenderKey) {
        // Only pin non-postWest survivors — never the free SAM hunter (v396 picket trap).
        const candidates = attackers.filter((tank) => {
        ...
        if (candidates.length > 0) {
          const key = objectKey(candidates[0]);
          clearMissionEightUnitRoleKey(key);
          state.baseGuardKeys.add(key);
      ...
      const weapCell = { cellX: 36, cellY: 55 };
      const weapThreatNear = hostiles.some((hostile) => (
        (hostile.type === 1 || hostile.type === 2)
        && missionEightDistance(hostile, weapCell) <= 10
      ));
      let needBase = Math.max(0, 1 - liveBaseTanks);
      if (rebuildReady && !weapThreatNear && freeProducedLive <= 1) {
        needBase = 0;
      }
      // Village MTNK only after free is mid-escape (y≤45) so pad keeps armor
      // until free@400 spawn (l2). Infantry still covers civs.
      let needVillage = 0;
      if (freePostWestCount >= 1 && freePastPad) {
        needVillage = Math.max(0, Math.min(1, villageTankTarget) - liveVillageTanks);
        if (rebuildReady && freeProducedLive <= 1) needVillage = 0;
      }
      const pool = attackers.filter((tank) => {
        ...
        return state.strikeKeys.has(key) || state.eastBProducedTankKeys.has(key)
          || (!state.eastBProducedTankKeys.has(key) && tank.cellY >= 40);
      }).toSorted((left, right) => (
        Number(state.eastBProducedTankKeys.has(objectKey(left)))
          - Number(state.eastBProducedTankKeys.has(objectKey(right)))
        || left.strength - right.strength
        || right.cellY - left.cellY
        || left.id - right.id
      ));
      ...
      const villagePool = attackers.filter((tank) => {
        if (tank.typeName !== "MTNK" || tank.strength < 20) return false;
        const key = objectKey(tank);
        if (key === state.eastBPostWestWeapDefenderKey) return false;
        if (key === state.eastBPostWestGunScrapKey) return false;
      ...
      // Demote non-produced strike survivors onto base (pad survival first —
      // l12 village dump left WEAP@241 at free spawn). Village gets staffed via
      // needVillage when base already has a defender.
      if (!rebuildReady) {
        for (const tank of attackers.toSorted((left, right) => (
          right.strength - left.strength || left.id - right.id
        ))) {
          if (tank.typeName !== "MTNK" || tank.strength < 20) continue;
          const key = objectKey(tank);
          if (!state.strikeKeys.has(key)) continue;
          if (state.eastBProducedTankKeys.has(key)) continue;
      ...
      if (needVillage > 0) {
        const refillCandidates = attackers.filter((tank) => {
          if (tank.typeName !== "MTNK" || tank.strength < 40) return false;
          const key = objectKey(tank);
          if (state.villageGuardKeys.has(key) || state.scoutKeys.has(key)) return false;
          const southern = tank.cellY >= 40 || tank.cellX <= 9;
          const weakStrike = state.strikeKeys.has(key)
            && tank.strength < Math.ceil(tank.maxStrength * 0.4);
          const freeProduced = state.eastBProducedTankKeys.has(key)
            && !state.strikeKeys.has(key);
          return southern || weakStrike || freeProduced || state.baseGuardKeys.has(key);
        }).toSorted((left, right) => (
          Number(state.strikeKeys.has(objectKey(right)))
            - Number(state.strikeKeys.has(objectKey(left)))
          || right.cellY - left.cellY
          || right.strength - left.strength
          || left.id - right.id
        ));
        for (const tank of refillCandidates) {
          if (needVillage <= 0) break;
          const key = objectKey(tank);
          clearMissionEightUnitRoleKey(key);
          state.villageGuardKeys.add(key);
  ...
  while (state.westCleanupStage < missionEightEastAWestCleanupTargets.length) {
    ...
    if (stage >= 3 && stage < 7 && state.postFactCleanupLaunchTick !== undefined
      ...
      const forceChargeAfterHold = holdDuration >= eastAProductionGunHoldTimeout(state)
        && productionGun.strength <= missionEightEastAProductionGunSoftStrength + 50
        ...
        || (eastAEarlyCapture(state) && sincePostHoldAirstrike !== undefined
          && sincePostHoldAirstrike >= 180
          && productionGun.strength <= missionEightEastAProductionGunSoftStrength)
      ...
      if (!forceChargeAfterHold && !readyToSurge) {
        ...
        const holdTarget = holdThreat ?? eastAProductionGunHoldCell(state);
    ...
    if (isProductionGun && state.productionGunChargeTick === undefined) {
      ...
      const previouslySoft = (state.productionGunMinimumStrength ?? 999)
        <= missionEightEastAProductionGunSoftStrength + 70;
      const forceChargeAfterHold = holdDuration >= eastAProductionGunHoldTimeout(state)
        && target.strength <= missionEightEastAProductionGunSoftStrength + 50
        || (eastAEarlyCapture(state) && sincePostHoldAirstrike !== undefined
          && sincePostHoldAirstrike >= 180
          && target.strength <= missionEightEastAProductionGunSoftStrength)
        || (sincePostHoldAirstrike !== undefined && sincePostHoldAirstrike >= 300
          && target.strength <= 300)
    ...
    if (stage >= 7) {
      ...
      if (stage === 7 && eastAEarlyCapture(state)
        && target.typeName === missionEightEastAProductionGunSite.typeName
        && target.cellX === missionEightEastAProductionGunSite.cellX
        && target.cellY === missionEightEastAProductionGunSite.cellY
        && target.strength > 0 && target.strength <= 40) {
      ...
      if (peeled.length > 0 && stage >= 8) {
        ...
        if (hand) {
          ...
          const diveElapsed = diveOrder !== undefined
            ...
            : -1;
          const diveOrderStrength = diveOrder?.targetStrength;
...
function queueMissionEightPostSamCounterattack(snapshot, hostiles, attackers, commands) {
  ...
  if (westernGun && westernGun.strength > 0
    ...
    for (const attacker of supportCandidates.slice(0, supportCap - state.postSamNorthSupportKeys.size)) {
      ...
      state.postSamNorthFlankKeys.delete(key);
      state.strikeKeys.delete(key);
      state.southReadyKeys.delete(key);
    }
  }
  ...
  for (let index = 0; index < engagementForce.length; index += 8) {
    ...
    queueMissionEightRole(commands,
      ...
      chunk, chunkTarget,
      chunkTarget === site ? MODIFIER_ALT : 0, 45);
  }
}
...
// After western SAM dies (v390), survivors must not solo-push NW SAM while the
// village is naked (TRACE v390: strike wiped by 33.6k, civ deaths from 34.5k).
// Rebuild cohort is free/strike tanks only — village keeps its own 2–3.
// l176k tried rebuild=2 SE hunter — free drifted east, western GUN never died
// (REGRESS). Keep 1 free for proven GUN→NE→NW; SE peel is free#1 after NW.
const eastBPostWestRebuildTankCount = 1;
const eastBPostWestSupportHold = { cellX: 13, cellY: 32 };

function eastBWesternSamDead(_hostiles = []) {
  // Only trust the death latch — never infer from a missing hostiles entry
  // (partial lists would false-trigger and divert the western assault).
  return missionEightState.samDeathTicks.has("13:16");
}

// Healthy post-west free tanks for rebuild/push. Only tanks that emerged after
// western SAM death count (TRACE v395c: pre-kill free false-ready'd and died).
// Counts base-staffed free so WEAP picket does not block ready (TRACE v395b).
function eastBHealthyOffVillageTanks(attackers) {
  return attackers.filter((attacker) => (
    attacker.typeName === "MTNK"
    && attacker.strength > 0
    && attacker.strength >= Math.ceil(attacker.maxStrength * 0.55)
    && missionEightState.eastBPostWestProducedTankKeys.has(objectKey(attacker))
    && !missionEightState.villageGuardKeys.has(objectKey(attacker))
  ));
}

function eastBWeapCell(friendly = []) {
  const weap = friendly.find((object) => (
    object.type === 4 && object.typeName === "WEAP" && object.strength > 0
  ));
  if (weap) {
    return {
      cellX: weap.cellX, cellY: weap.cellY,
      strength: weap.strength, maxStrength: weap.maxStrength, known: true,
    };
  }
  // Unknown (caller omitted buildings) — do not treat as dead.
  return {
    cellX: 36, cellY: 55, strength: 400, maxStrength: 400, known: false,
  };
}

function eastBWeapThreatNear(hostiles, weapCell, radius = 12) {
  return hostiles.some((hostile) => (
    (hostile.type === 1 || hostile.type === 2)
    && missionEightDistance(hostile, weapCell) <= radius
  ));
}

function eastBNearestWeapThreat(hostiles, weapCell, radius = 12) {
  return hostiles.filter((hostile) => (
    (hostile.type === 1 || hostile.type === 2)
    && missionEightDistance(hostile, weapCell) <= radius
  )).toSorted((left, right) => (
    missionEightDistance(left, weapCell) - missionEightDistance(right, weapCell)
    || left.strength - right.strength
    || left.id - right.id
  ))[0];
}

// Factory is secure enough to release free armor for remaining SAMs.
// TRACE l1: free@36600 WEAP@400 still sat picket 300t (threat radius) then left
// mid-pad — if WEAP is healthy, free leaves immediately; pad fire is the
// pinned defender's job.
function eastBWeapSecureForSamPush(hostiles, friendly = [], tick = 0) {
  const weapLive = friendly.length > 0
    ? friendly.find((object) => (
      object.type === 4 && object.typeName === "WEAP" && object.strength > 0
    ))
    : undefined;
  if (friendly.length > 0 && !weapLive) {
    // WEAP confirmed dead — push SAMs (only chance left).
    return true;
  }
  const weap = eastBWeapCell(friendly);
  // WEAP healthy (≥80%) → free leaves now even if pad threats exist.
  if (weap.known && weap.strength >= Math.ceil(weap.maxStrength * 0.8)) {
    return true;
  }
  // WEAP critically hurt — free cannot save it; go kill SAMs.
  if (weap.known && weap.strength > 0
    && weap.strength < Math.ceil(weap.maxStrength * 0.65)) {
    return true;
  }
  // Timed release after 200 ticks if still held for mid-band WEAP HP.
  const emerge = missionEightState.eastBPostWestFreeEmergeTick;
  if (emerge !== undefined && tick > 0 && tick - emerge >= 200) {
    return true;
  }
  if (weap.known && weap.strength < Math.ceil(weap.maxStrength * 0.8)) return false;
  if (eastBWeapThreatNear(hostiles, weap, 6)) return false;
  return true;
}

// Post-west free tanks for remaining-SAM combat only.
// TRACE v395b: sole free sat on WEAP picket forever.
// TRACE v395n: free left while WEAP dying → pad raiders killed free + WEAP.
// Phase: free pickets/clears WEAP first; release to strike only when factory secure.
function eastBPostWestPromoteCombatTanks(attackers, hostiles = [], friendly = [],
  tick = 0) {
  const state = missionEightState;
  const rebuildReady = state.eastBPostWestRebuildReadyTick !== undefined;
  const weap = eastBWeapCell(friendly);
  const weapThreat = eastBWeapThreatNear(hostiles, weap, 12);
  // tick=0 in assignRoles (no timeout); push path passes snapshot.tick.
  const weapSecure = eastBWeapSecureForSamPush(hostiles, friendly, tick);
  let baseMtnk = attackers.filter((unit) => (
    unit.typeName === "MTNK" && state.baseGuardKeys.has(objectKey(unit))
    && unit.strength > 0
  ));
  const freePostWest = attackers.filter((unit) => (
    unit.typeName === "MTNK"
    && state.eastBPostWestProducedTankKeys.has(objectKey(unit))
    && !state.villageGuardKeys.has(objectKey(unit))
    && !state.baseGuardKeys.has(objectKey(unit))
  ...
  // l181: free mid-map / SE peel (x up to 40, y≤40) must stay on strike after
  // GUN clear — do not re-picket WEAP while SE SAMs still live.
  const freeOnSpinePush = (tank) => (
    !westernGunLiveForPromote
    && tank.cellX <= 48 && tank.cellY <= 40 && tank.strength >= 40
  );
  ...
  if (rebuildReady && weapSecure && baseMtnk.length >= 1) {
    for (const tank of baseMtnk) {
      const key = objectKey(tank);
      if (!state.eastBPostWestProducedTankKeys.has(key)) continue;
      if (tank.strength < Math.ceil(tank.maxStrength * 0.7)) continue;
      state.baseGuardKeys.delete(key);
      state.strikeKeys.add(key);
    }
    baseMtnk = attackers.filter((unit) => (
  ...
  for (const tank of attackers) {
    ...
    if (!state.eastBPostWestProducedTankKeys.has(key)) continue;
    if (state.villageGuardKeys.has(key)) continue;
    // Hold base picket while factory not secure.
    if (state.baseGuardKeys.has(key) && !weapSecure) continue;
    if (state.baseGuardKeys.has(key) && (!rebuildReady || !weapSecure)) continue;
    if (state.scoutKeys.has(key)) continue;
    const freeNow = !state.villageGuardKeys.has(key) && !state.baseGuardKeys.has(key);
    // Do not promote free→strike while WEAP needs the free tank.
    // l181: never demote free already on remaining-SAM push (spine / SE peel).
    if (freeNow && !weapSecure && weap.strength > 0 && !freeOnSpinePush(tank)) {
      state.baseGuardKeys.add(key);
      state.strikeKeys.delete(key);
      continue;
    }
    if (!state.strikeKeys.has(key) && !freeNow) continue;
    if (freeNow) {
      state.strikeKeys.add(key);
      state.baseGuardKeys.delete(key);
      state.villageGuardKeys.delete(key);
    }
    if (state.strikeKeys.has(key)) combat.push(tank);
  }
  return combat;
}

function eastBWeapAlive(friendlyOrAttackers = []) {
  // Prefer full friendly list; fall back to any object array with type 4.
  return friendlyOrAttackers.some((object) => (
    object.type === 4 && object.typeName === "WEAP" && object.strength > 0
  ));
}

function eastBPostWestNeedsRebuild(attackers, hostiles, friendly = [], tick = 0) {
  if (mission.variant !== "east-b") return false;
  if (!eastBWesternSamDead(hostiles)) return false;
  if (missionEightState.allSamsDeadTick !== undefined) return false;
  if (missionEightState.eastBPostWestRebuildReadyTick !== undefined) return false;
  const freeHealthy = eastBHealthyOffVillageTanks(attackers).length;
  if (freeHealthy >= eastBPostWestRebuildTankCount
    && eastBWeapSecureForSamPush(hostiles, friendly, tick)) {
    return false;
  }
  // Stay in rebuild while free cohort thin or WEAP still needs a free picket.
  return true;
}

function eastBPostWestMarkRebuildReady(attackers, hostiles, tick, friendly = []) {
  if (mission.variant !== "east-b") return;
  ...
  // Ready when free tank exists AND factory is secure (or picket timeout).
  if (eastBHealthyOffVillageTanks(attackers).length < eastBPostWestRebuildTankCount) {
    return;
  }
  if (!eastBWeapSecureForSamPush(hostiles, friendly, tick)) return;
...
function queueEastBSamPostWesternSamPush(commands, snapshot, hostiles, strike, attackers = strike,
  friendly = []) {
  if (mission.variant !== "east-b") return false;
  const state = missionEightState;
  if (!state.samDeathTicks.has("13:16")) return false;
  // Only block while the western SAM is still alive — a strength-0 corpse left
  // in the hostiles list used to short-circuit the entire remaining-SAM push
  // (TRACE v395d free strike tank walked south to the village instead of NW).
  if (hostiles.some((hostile) => (
    hostile.typeName === "SAM" && hostile.cellX === 13 && hostile.cellY === 16
    && hostile.strength > 0
  ))) return false;
  if (state.routeStage < 7) return false;
  const friendlies = friendly.length > 0 ? friendly : snapshot.objects.filter((object) => (
    object.owner === HOUSE_GDI && object.subObject === 0 && object.strength > 0
  ));
  // Promote free produced tanks into strike before any push (TRACE v394f/v395b
  // free=1 strike=0 — combat force never left the factory pad).
  eastBPostWestMarkRebuildReady(attackers, hostiles, snapshot.tick, friendlies);
  let tanks = eastBPostWestPromoteCombatTanks(attackers, hostiles, friendlies,
    snapshot.tick);
  if (tanks.length === 0 && eastBWeapSecureForSamPush(hostiles, friendlies, snapshot.tick)) {
    tanks = attackers.filter((attacker) => (
      attacker.typeName === "MTNK"
      && attacker.strength >= Math.ceil(attacker.maxStrength * 0.55)
      && state.eastBPostWestProducedTankKeys.has(objectKey(attacker))
      && !state.villageGuardKeys.has(objectKey(attacker))
    ));
    for (const tank of tanks) {
      const key = objectKey(tank);
  ...
  if (tanks.length === 0) {
    tanks = strike.filter((attacker) => (
      attacker.typeName === "MTNK" && attacker.strength > 0
      && state.eastBPostWestProducedTankKeys.has(objectKey(attacker))
    ));
  }
  // Rebuild / WEAP-secure hold: free picket intercepts raiders at standoff
  // (not on the WEAP pad — TRACE v396 free-on-pad attracted fire into WEAP).
  if (eastBPostWestNeedsRebuild(attackers, hostiles, friendlies, snapshot.tick)
    || !eastBWeapSecureForSamPush(hostiles, friendlies, snapshot.tick)) {
    const weap = friendlies.find((object) => (
      object.type === 4 && object.typeName === "WEAP" && object.strength > 0
    ));
    const weapPicket = weap
      ? { cellX: weap.cellX - 2, cellY: weap.cellY - 2 }
      : { cellX: 32, cellY: 52 };
    const weapCell = weap
      ? { cellX: weap.cellX, cellY: weap.cellY }
      : { cellX: 36, cellY: 55 };
    const padThreat = eastBNearestWeapThreat(hostiles, weapCell, 14);
    // Free post-west tanks: intercept raiders NW of the pad, not on top of WEAP.
    const freePicket = attackers.filter((tank) => (
      tank.typeName === "MTNK"
      && state.eastBPostWestProducedTankKeys.has(objectKey(tank))
      && (state.baseGuardKeys.has(objectKey(tank))
        || !state.strikeKeys.has(objectKey(tank)))
      && tank.strength > 0
    ));
    for (const tank of freePicket) {
      if (state.strikeKeys.has(objectKey(tank))
        && eastBWeapSecureForSamPush(hostiles, friendlies, snapshot.tick)) {
        continue; // already released for SAM
      }
      const tankKey = objectKey(tank);
      if (!state.baseGuardKeys.has(tankKey)) {
        state.baseGuardKeys.add(tankKey);
        state.strikeKeys.delete(tankKey);
      }
      if (padThreat && missionEightDistance(tank, padThreat) <= 6) {
        queueMissionEightRole(commands, `east-b-post-west-pad-clear-${tankKey}`,
          [tank], padThreat, 0, 1);
      } else if (padThreat) {
        // Intercept between raider and WEAP, not under the factory.
        const intercept = {
          cellX: Math.round((padThreat.cellX + weapCell.cellX) / 2),
          cellY: Math.round((padThreat.cellY + weapCell.cellY) / 2),
        };
        queueMissionEightRole(commands, `east-b-post-west-pad-intercept-${tankKey}`,
          [tank], intercept, MODIFIER_ALT, 3);
      } else if (missionEightDistance(tank, weapPicket) > 2) {
        queueMissionEightRole(commands, `east-b-post-west-weap-hold-${tankKey}`,
          [tank], weapPicket, MODIFIER_ALT, 10);
      }
    }
    // Non-post-west scrap: support hold if on spine, else WEAP.
    for (const tank of attackers) {
      if (tank.typeName !== "MTNK" || tank.strength <= 0) continue;
      const key = objectKey(tank);
      if (state.eastBPostWestProducedTankKeys.has(key)) continue;
      if (state.villageGuardKeys.has(key)) continue;
      if (state.strikeKeys.has(key)) {
        state.strikeKeys.delete(key);
        state.baseGuardKeys.add(key);
      }
      const onWesternSpine = tank.cellX <= 20 && tank.cellY <= 45;
      const rally = onWesternSpine ? eastBPostWestSupportHold : weapPicket;
      if (missionEightDistance(tank, rally) > 2) {
        queueMissionEightRole(commands, `east-b-post-west-rebuild-${key}`,
  ...
  // Force-move approach stages after west SAM also wait for rebuild — but never
  // park post-west free tanks on the civil intercept.
  if (waypoint?.forceMove && state.eastBPostWestRebuildReadyTick === undefined
    && eastBHealthyOffVillageTanks(attackers).length < eastBPostWestRebuildTankCount
    && tanks.length === 0) {
    return false;
  }
  // Prefer route SAM target; else remaining live SAM. NW 12,5 first while up.
  // l176: after NW dead free peels SE — pick nearest live SE SAM to free
  // (site order 54,5 first sent free to far NE corner; 43,14 is closer).
  let target;
  if (waypoint?.typeName === "SAM") {
    const targetCellX = waypoint.targetCellX ?? waypoint.cellX;
    const targetCellY = waypoint.targetCellY ?? waypoint.cellY;
    target = hostiles.find((hostile) => (
      hostile.typeName === "SAM"
      && hostile.cellX === targetCellX && hostile.cellY === targetCellY
      && hostile.strength > 0
    ));
  ...
  if (!target) {
    const nwLive = hostiles.find((hostile) => (
      hostile.typeName === "SAM" && hostile.cellX === 12 && hostile.cellY === 5
      && hostile.strength > 0
    ));
    if (nwLive) {
      target = nwLive;
    } else {
      const freeAnchor = tanks.filter((tank) => (
        state.eastBPostWestProducedTankKeys.has(objectKey(tank))
        && tank.strength > 0
      )).toSorted((a, b) => a.cellY - b.cellY || b.strength - a.strength)[0]
        ?? tanks[0];
      const seSams = hostiles.filter((hostile) => (
        hostile.typeName === "SAM" && hostile.strength > 0
        && !(hostile.cellX === 13 && hostile.cellY === 16)
      ));
      if (freeAnchor && seSams.length > 0) {
        target = seSams.toSorted((left, right) => (
          missionEightDistance(freeAnchor, left) - missionEightDistance(freeAnchor, right)
          || left.cellX - right.cellX
          || left.id - right.id
        ))[0];
      } else {
        for (const site of missionEightSamSites["east-b"]) {
          const sam = hostiles.find((hostile) => (
            hostile.typeName === "SAM"
            && hostile.cellX === site.cellX && hostile.cellY === site.cellY
            && hostile.strength > 0
          ));
  ...
  // l488: residual free path WHILE SE SAMs still live. TRACE l479 residual@38700
  // with SE SAMs@400 — free rails pad {38,11}; free-near chips SE SAMs + pad.
  // freeForce/healthyEastFree used to sit only inside if (!target) (allSamsDead)
  // so residual never latched early → freeLast@41k (l486/l488). Latch + rail
  // here; once free holds pad free-near finishes SAMs then if (!target) runs.
  {
    const residualFree = attackers.filter((unit) => (
      unit.typeName === "MTNK"
      && unit.strength >= 40
      && (state.eastBPostWestProducedTankKeys.has(objectKey(unit))
        || state.strikeKeys.has(objectKey(unit)))
    ...
    for (const tank of residualFree) {
      const healthyEastFree = tank.strength >= 100
        && tank.cellY <= 25
        && tank.cellX >= 20
        && snapshot.tick >= 38_500
        && (state.allSamsDeadTick !== undefined
          || state.routeStage >= 8
          || state.eastBPostWestGunClearedTick !== undefined);
      if (healthyEastFree) state.eastBResidualCommit = true;
    }
    // l519dy mid-screen CLOSED (lose@59257 free 170→118). Keep pre-allsam
    // softHold override for free@pad path (da@60138).
    if (state.eastBResidualCommit && target && residualFree.length > 0) {
      // SAMs still live: rail/hold sole healthy free on pad (no SE SAM walk).
      // l491: do NOT override free before x≥26 — l479 free already@28,18@38700
      // via SE SAM corridor with str391; early override from x~22 (l489/l490)
      // bled free through ARTY/LTNK freeLast@41k. Latch residual early for
      // free-near, but let SAM path push free east until mid-corridor, then
      // peel free off SE SAM fire cells onto pad.
      const soleFree = residualFree
        .filter((t) => t.cellY <= 28 && t.cellX >= 18 && t.strength >= 100)
        .toSorted((a, b) => b.strength - a.strength
          || a.cellY - b.cellY
          || b.cellX - a.cellX)[0];
      let ordered = 0;
      // Gate: free must reach mid-east corridor before pad override.
      if (soleFree && (soleFree.cellX >= 26 || soleFree.cellY <= 15)) {
        const tank = soleFree;
        const key = objectKey(tank);
        clearMissionEightUnitRoleKey(key);
        state.strikeKeys.add(key);
        state.baseGuardKeys.delete(key);
        if (tank.cellX < 36 && tank.cellY <= 28) {
          // North corridor: climb to y≤14 first, then east (avoid ARTY band).
          const eastRail = tank.cellY > 15
            ? { cellX: Math.min(34, Math.max(tank.cellX, 24)), cellY: 13 }
            : { cellX: 40, cellY: 12 };
          const nearArty = hostiles.filter((h) => (
            h.strength > 0 && h.typeName === "ARTY"
            && missionEightDistance(tank, h) <= 4
          )).toSorted((a, b) => (
            missionEightDistance(tank, a) - missionEightDistance(tank, b)
          ))[0];
          // Soft scrap ARTY only when adj-ish and free healthy; else north-kite.
          if (nearArty && tank.strength >= 150
            && missionEightDistance(tank, nearArty) <= 2) {
            queueMissionEightRole(commands, `east-b-pre-allsam-arty-scrap-${key}`,
              [tank], nearArty, 0, 1);
          } else if (nearArty) {
            const northKite = {
              cellX: Math.min(36, tank.cellX + 2),
              cellY: Math.max(11, Math.min(13, tank.cellY - 2)),
            };
            queueMissionEightRole(commands, `east-b-pre-allsam-arty-kite-${key}`,
              [tank], northKite, MODIFIER_ALT, 1);
          } else {
            ...
            if (chaseArmor && tank.strength >= 80) {
              if (missionEightDistance(tank, chaseArmor) <= 1) {
                queueMissionEightRole(commands, `east-b-pre-allsam-east-adj-${key}`,
                  [tank], chaseArmor, 0, 1);
              } else {
                const stepEast = {
                  cellX: Math.min(42, tank.cellX + 3),
                  cellY: Math.max(11, Math.min(14, tank.cellY)),
                };
                queueMissionEightRole(commands, `east-b-pre-allsam-east-kite-${key}`,
                  [tank], stepEast, MODIFIER_ALT, 1);
              }
            } else {
              queueMissionEightRole(commands, `east-b-pre-allsam-east-rail-${key}`,
                [tank], eastRail, MODIFIER_ALT, 2);
            }
          }
          ordered += 1;
        } else if (tank.cellX >= 36 && tank.cellY <= 22) {
          // l519b: restore l512 melee str≥120 — peel thrash free@58 (l519).
          // l519dn: restore l519dj softHold {38,11} pre-allsam (best@62189
          // was accidental y16 from leave holdCell clamp + door thrash).
          const softHold = { cellX: 38, cellY: 11 };
          const ltnkAdj = hostiles.filter((h) => (
            h.strength > 0
            && (h.typeName === "LTNK" || h.typeName === "BGGY")
            && missionEightDistance(tank, h) <= 1
          )).toSorted((a, b) => a.strength - b.strength)[0];
          if (ltnkAdj && tank.strength >= 120) {
            queueMissionEightRole(commands, `east-b-pre-allsam-ltnk-adj-${key}`,
              [tank], ltnkAdj, 0, 1);
          } else if (ltnkAdj && tank.strength < 120) {
            queueMissionEightRole(commands, `east-b-pre-allsam-ltnk-peel-${key}`,
              [tank], softHold, MODIFIER_ALT, 1);
          } else if (missionEightDistance(tank, softHold) > 2) {
            queueMissionEightRole(commands, `east-b-pre-allsam-pad-hold-${key}`,
              [tank], softHold, MODIFIER_ALT, 2);
          } else {
            queueMissionEightRole(commands, `east-b-pre-allsam-pad-park-${key}`,
          ...
          if (!eastPadProdNow && !eastPadGunsNow && tank.cellX >= 30) {
      ...
      if (ordered > 0) {
        // l494: early residual return skipped SE village screen + main village
        // AI (caller returns). TRACE l493 vg 4→0@39600 neutMin 9→6 by 41400
        // while l479 held vg=5 neutMin=8. Re-staff villageGuardKeys from village
        // infantry and run full hospital screen before return.
        const hospHold = { cellX: 5, cellY: 58 };
        const midHold = { cellX: 12, cellY: 52 };
        const villageInf = attackers.filter((u) => (
          u.type === 1 && u.strength > 0
          && (state.villageGuardKeys.has(objectKey(u))
            || (u.cellY >= 48 && u.cellX <= 22)
            || (u.cellY >= 52 && u.cellX <= 28))
        ));
        for (const u of villageInf) {
          state.villageGuardKeys.add(objectKey(u));
          state.strikeKeys.delete(objectKey(u));
        }
        const screen = attackers.filter((u) => (
          u.strength > 0 && state.villageGuardKeys.has(objectKey(u))
        )).slice(0, 14);
        if (screen.length > 0) {
          // l519n: free@170@pad; residual return skips main Moebius picket
          // (queueEastB true → queueMissionEightForces returns). This block is
          // the only village AI after residual free soft-hold.
          // l519av: l519at los3-moebius@47495 after vg 6→0@46800 — lateWave
          // 2/3 hunters emptied Moebius body. Keep ≥3 body on Moebius pad
          // {6,58}; cap hunters to 1 late (tick≥46k) or ceil(n/3) earlier;
          // only engage TRAN/E4 within d≤8 (not d14 wander-to-death).
          const moebiusPad = { cellX: 6, cellY: 58 };
          const nearHosp = hostiles.filter((h) => (
            h.strength > 0
            && (h.type === 1 || h.type === 2 || h.typeName === "TRAN")
            && missionEightDistance(h, hospHold) <= 16
          )).toSorted((a, b) => {
            const rank = (h) => (
              h.typeName === "TRAN" ? 0
                : h.typeName === "E4" ? 1
            ...
            return rank(a) - rank(b)
              || missionEightDistance(a, hospHold) - missionEightDistance(b, hospHold)
              || b.strength - a.strength;
          })[0];
          const dHosp = nearHosp ? missionEightDistance(nearHosp, hospHold) : 99;
          const airliftThreat = nearHosp
            && (nearHosp.typeName === "TRAN" || nearHosp.typeName === "E4");
          // l519bl: pure body after tick≥50k (hold Moebius/HOSP only).
          // l519bx/cj late hunters regressed civ wall — pure body kept (l519cg).
          // l519do CLOSED: free MTNK late door after 57k → civ-near@59138.
          // l519ec/ee TRAN scrap/picket no-op (peels die@door before TRAN).
          // l519ef: engine post-pass3 3.5 min rearm → pass4 HAND@~57k.
          const pureBody = snapshot.tick >= 50_000;
          const pass2Window = snapshot.tick >= 46_000;
          const engageRange = pureBody
            ? (airliftThreat ? 5 : 3)
            : airliftThreat
              ? (pass2Window ? 8 : 10)
              : 5;
          const engage = nearHosp && dHosp <= engageRange && !pureBody;
          const byStr = screen.toSorted((a, b) => b.strength - a.strength);
          if (engage) {
            const bodyMin = Math.min(
              byStr.length,
              Math.max(3, Math.ceil(byStr.length * 0.6)),
            );
            const holders = byStr.slice(0, bodyMin);
            const maxHunt = pass2Window ? 1 : Math.max(1, Math.ceil(screen.length / 3));
            const hunters = byStr.filter((u) => (
              !holders.includes(u) && u.strength >= 12
            )).slice(0, maxHunt);
            if (hunters.length === 0 && airliftThreat && dHosp <= 6
              && byStr.length > 3) {
              hunters.push(byStr[byStr.length - 1]);
            }
            if (hunters.length > 0) {
              queueMissionEightRole(commands, "east-b-residual-village-screen",
                hunters, nearHosp, 0, airliftThreat ? 4 : 5);
            }
            const bodyHold = holders.filter((u) => !hunters.includes(u));
            if (bodyHold.length > 0) {
              const moeN = Math.ceil(bodyHold.length / 2);
              queueMissionEightRole(commands, "east-b-residual-moebius-body",
                bodyHold.slice(0, moeN), moebiusPad, MODIFIER_ALT, 8);
              if (bodyHold.length > moeN) {
                queueMissionEightRole(commands, "east-b-residual-hosp-body",
                  bodyHold.slice(moeN), hospHold, MODIFIER_ALT, 8);
              }
            }
          } else {
            // Pure body / no threat: all on Moebius + HOSP.
            // If airlift on door (d≤5) after 50k, 1 scrap peels only.
            // l519dq/du static picket CLOSED (no change@60138). l519do hunt CLOSED.
            if (pureBody && airliftThreat && dHosp <= 5 && byStr.length > 2) {
              const scrap = byStr[byStr.length - 1];
              queueMissionEightRole(commands, "east-b-residual-village-scrap",
                [scrap], nearHosp, 0, 4);
              const body = byStr.slice(0, -1);
              const moeN = Math.ceil(body.length / 2);
              queueMissionEightRole(commands, "east-b-residual-moebius-body",
                body.slice(0, moeN), moebiusPad, MODIFIER_ALT, 6);
              if (body.length > moeN) {
                queueMissionEightRole(commands, "east-b-residual-hosp-body",
                  body.slice(moeN), hospHold, MODIFIER_ALT, 6);
              }
            } else {
              const moeN = Math.ceil(screen.length / 2);
              queueMissionEightRole(commands, "east-b-residual-moebius-body",
                screen.slice(0, moeN), moebiusPad, MODIFIER_ALT, 8);
              if (screen.length > moeN) {
                queueMissionEightRole(commands, "east-b-residual-hosp-body",
                  screen.slice(moeN), hospHold, MODIFIER_ALT, 8);
              }
            }
          }
        }
  ...
  if (!target) {
    // l176v/l178: all SAMs dead — free east assault with AFLD *standoff*
    // (point-blank attack-move into GUN@41–42,5 killed free before AFLD died).
    // Hold on south fire cells so engine proximity chips AFLD while free lives.
    state.allSamsDeadTick ??= snapshot.tick;
    // l198/l199/l200: explicit east-b A-10 multi-pass after all SAMs.
    // Priority AFLD→HAND→FACT on the *eastern* base (x≥40) — first pass used
    // global HAND@5,12 (west) and HAND stayed 800 with no base damage.
    // Re-order whenever SW_AirStrike.completed again (!pending) — recharge is
    // AIR_CANNON_GONE_TIME (8 min ≈ 7200t); village must hold until re-ready.
    // Min 90t between orders to avoid spam while completed flickers.
    const airstrikeEntry = snapshot.sidebar?.entries?.find((entry) => (
      entry.assetName === "SW_AirStrike"
    ));
    ...
    // l438j–l458 freeLast@83164 path: pad ARTY/BGGY first, then HAND, hospital
    // E4 pack (tick≥60k pass≥3), then TRAN. Restored after accidental checkout.
    ...
    // l519ec: y≤16 free-near band (da). Mid free@y25 intentionally NOT air
    // anchor — pass3 falls through to door pack (eb HAND@mid CLOSED@55693).
    const freePadHoldAir = attackers.find((u) => (
      u.typeName === "MTNK" && u.strength >= 5
      && u.cellX >= 30 && u.cellY <= 16
      && (state.eastBPostWestProducedTankKeys.has(objectKey(u))
        || state.strikeKeys.has(objectKey(u)))
    ));
    ...
    const airOk = airPassCount < 2
      || Boolean(state.eastBResidualCommit)
      || freeResidualAir;
    const padLocalArmor = freePadHoldAir && hostiles.some((h) => (
      h.strength > 0 && (h.typeName === "LTNK" || h.typeName === "BGGY")
      && missionEightDistance(freePadHoldAir, h) <= 6
    ));
    const padArtyNear = freePadHoldAir && hostiles.some((h) => (
      h.strength > 0 && h.typeName === "ARTY" && h.cellX >= 28
      && h.cellX <= 50 && h.cellY >= 10 && h.cellY <= 22
      && missionEightDistance(freePadHoldAir, h) <= 8
    ));
    // l519l: no pass2 hold when free@pad healthy — rearm (~7200) is the gate;
    // fire pass2 hospital ASAP (civ-nine@47k with free@170 still live l519j).
    const pass2HoldForPadThreat = airPassCount === 1
      && Boolean(state.eastBEastPadClearedTick)
      && Boolean(freePadHoldAir)
      && freePadHoldAir.strength < 100
      && !padLocalArmor
      && !padArtyNear
      && snapshot.tick < (state.eastBEastPadClearedTick + 3_500);
    // l507: hold pass1 until free on softHold band (x≥36 y≤16). Ordering A-10
    // while free mid-approach@33 ranks ARTY while LTNK d=4 on free → free dies
    // under LTNK before bomb lands. Wait free@pad for honest pad-threat rank.
    const pass1WaitFreeOnPad = airPassCount === 0
      && Boolean(state.eastBResidualCommit)
      && !(freePadHoldAir
        && freePadHoldAir.cellX >= 36
        && freePadHoldAir.cellY <= 16);
    // l519ef: pass4 ready@~57270 HAND undamaged → lose@59258.
    // l519eg keeper: hold pass4 for any door-band TRAN → lose@62248 (+2.1k).
    // l519eh tighter TRAN band CLOSED (lose@59523). Fallback tick≥59800.
    const doorTranForAir = hostiles.some((h) => (
      h.strength > 0
      && h.typeName === "TRAN"
      && h.cellY >= 56
      && h.cellX >= 16
    ...
    const pass4HoldForDoorTran = airPassCount >= 3
      && snapshot.tick < 59_800
      && !doorTranForAir;
    if (airstrikeEntry?.completed && !state.airstrike.pending
      && snapshot.tick >= state.allSamsDeadTick + 15
      && snapshot.tick >= lastAirOrderTick + 90
      && airOk
      && !pass2HoldForPadThreat
      && !pass1WaitFreeOnPad
      && !pass4HoldForDoorTran) {
      const orderedTypes = new Set(
        (state.airstrike.orders ?? []).map((o) => o.target),
      );
      const orderedCells = new Set(
        (state.airstrike.orders ?? []).map((o) => `${o.cellX},${o.cellY}`),
      );
      const freeAirAnchor = freePadHoldAir;
      const padClearedAir = state.eastBEastPadClearedTick !== undefined;
      // l438j: include east-theatre ARTY x≥28 (not fog ARTY@56,5). l450: hospital
      // TRAN / E4 pack late civ screen. West HAND/FACT always eligible.
      const eastBase = hostiles.filter((h) => {
        if (h.strength <= 0) return false;
        if (h.typeName === "ARTY" && h.cellX >= 28 && h.cellX <= 50
          && h.cellY >= 10 && h.cellY <= 22) {
          return true;
        }
        if (freeAirAnchor && (h.typeName === "LTNK" || h.typeName === "BGGY")
          && missionEightDistance(freeAirAnchor, h) <= 6) {
          return true;
        }
        // Hospital pack eligible after pad clear OR residual (even if free dies).
        // l519as: widen TRAN band y≥48 (was ≥55) — airlifts stage mid-village
        // before drop; pass2 missed TRAN and hit E4@4,55 only.
        if (padClearedAir || Boolean(state.eastBResidualCommit)) {
          if (h.typeName === "TRAN" && h.cellY >= 48 && h.cellX >= 8
            && h.cellX <= 32) {
            return true;
          }
          if ((h.typeName === "E4" || h.typeName === "E3" || h.typeName === "E1")
            && h.cellY >= 52 && h.cellX <= 26) {
            return true;
          }
        }
        ...
        if (h.cellX >= 40 && (
          h.typeName === "AFLD" || h.typeName === "PROC" || h.typeName === "GUN"
          || h.typeName === "NUKE" || h.typeName === "LTNK"
        )) {
          return true;
        }
        return false;
      ...
      const airTarget = (eastBase.length > 0 ? eastBase : hostiles.filter((h) => (
        ...
        const rank = (u) => {
          // l519aa keeper: pass1 pad LTNK (free@170 stable; hospital-first
          // free dies@41k). l519ds/dt CLOSED hospital pass1 → free@42k.
          // Pass2+ hospital E4. free@pad no peel.
          const passCountNowEarly = state.airstrike.orders?.length ?? 0;
          if (passCountNowEarly < 1 && freeAirAnchor
            && (u.typeName === "LTNK" || u.typeName === "BGGY")
            && missionEightDistance(freeAirAnchor, u) <= 5) {
            return -12;
          }
          if (freeAirAnchor && u.typeName === "ARTY"
            && u.cellX >= 28 && u.cellX <= 50 && u.cellY >= 10 && u.cellY <= 22) {
            return -10;
          }
          if (passCountNowEarly >= 1 && freeAirAnchor
            && freeAirAnchor.strength < 150
            && (u.typeName === "LTNK" || u.typeName === "BGGY")
            && missionEightDistance(freeAirAnchor, u) <= 5) {
            return -11;
          }
          const artyLiveNear = hostiles.some((h) => (
            h.strength > 0 && h.typeName === "ARTY" && h.cellX >= 28
            && h.cellX <= 50 && h.cellY >= 10 && h.cellY <= 22
          ));
          const ltnkLiveNear = freeAirAnchor && hostiles.some((h) => (
            h.strength > 0 && (h.typeName === "LTNK" || h.typeName === "BGGY")
          ...
          if (freeAirAnchor && freeAirAnchor.cellX >= 30 && freeAirAnchor.cellY <= 16
            ...
            // y≤16 str≥150 (rearm ~61290). free mid never freeOkHand.
            const freeOkHand = freeAirAnchor.strength >= 150
              && freeAirAnchor.cellX >= 36 && freeAirAnchor.cellY <= 16;
            // pass4+ (l519eg keeper lose@62248): door-band TRAN first, then E4,
            // HAND. Tighter TRAN band l519eh CLOSED.
            ...
            if (passCountNow >= 3 && (freeOkHand || lateDoorAir || lateResidualAir)) {
              ...
              if (!lateDoorAir || freeOkHand) {
                ...
                if (u.typeName === "FACT") return -21;
                if (u.typeName === "HQ") return -20;
              }
              if (u.typeName === "TRAN" && u.cellY >= 48 && u.cellX <= 32) {
                return -19;
              }
              if (u.typeName === "GUN" && lateDoorAir) return -18;
              if (u.typeName === "PROC") return -16;
              if (lateDoorAir && u.typeName === "HQ") return -17;
              if (lateDoorAir && u.typeName === "NUKE") return -15;
            }
            if (passCountNow >= 1 && passCountNow < 2) {
              // pass2 hospital ONLY. l519cx door-prefer pass2 CLOSED.
              const clusterNear = (u) => hostiles.filter((h) => (
                h.strength > 0
                && (h.typeName === "E4" || h.typeName === "E3"
                  || h.typeName === "E1" || h.typeName === "TRAN")
                && missionEightDistance(u, h) <= 3
              )).length;
              if ((u.typeName === "E4" || u.typeName === "E3" || u.typeName === "E1")
                && u.cellY >= 52 && u.cellX <= 26) {
                const cl = clusterNear(u);
                if (u.typeName === "E4") {
                  return u.strength >= 30 ? -17 - Math.min(cl, 3) : -13;
                }
                if (u.typeName === "E3") {
                  return u.strength >= 20 ? -15 - Math.min(cl, 2) : -12;
                }
                if (u.typeName === "E1") return -12;
              }
              if (u.typeName === "TRAN" && u.cellY >= 48 && u.cellX <= 32) {
                return -11 - Math.min(clusterNear(u), 2);
              }
            }
            // pass3 door pack (and pass4+ when free not softHold for HAND).
            if (passCountNow >= 2 && !(passCountNow >= 3 && freeOkHand)) {
              const clusterNear = (u) => hostiles.filter((h) => (
                h.strength > 0
                && (h.typeName === "E4" || h.typeName === "E3"
                  || h.typeName === "E1" || h.typeName === "TRAN")
            ...
            // Pad LTNK only pass1 (or pass2+ when free scrap <150).
            if (ltnkLiveNear
              && (passCountNow < 1
                || (passCountNow >= 1 && freeAirAnchor.strength < 150))) {
              if ((u.typeName === "LTNK" || u.typeName === "BGGY")
                && missionEightDistance(freeAirAnchor, u) <= 6) {
                return -8;
              }
            }
            if (!(ltnkLiveNear && passCountNow < 1)) {
              if (u.typeName === "HAND") return -5;
              if (u.typeName === "FACT") return -4;
              if (u.typeName === "HQ") return -3;
              if (u.typeName === "AFLD") return -2;
              if (u.typeName === "PROC") return -1;
            }
          }
          // l498: free dead — still hospital pack before HAND (civ-nine wall).
          // freeAirAnchor null skips block above; residual/allSams still need
          // pass2+ E4/TRAN for Moebius/HOSP screen.
          if (!freeAirAnchor && passCountNow >= 1
            && Boolean(state.eastBResidualCommit || state.allSamsDeadTick !== undefined)) {
            if (u.typeName === "TRAN" && u.cellY >= 55 && u.cellX >= 10
              && u.cellX <= 28) {
              return -12;
            }
            if ((u.typeName === "E4" || u.typeName === "E3" || u.typeName === "E1")
              && u.cellY >= 55 && u.cellX <= 24) {
              if (u.typeName === "E4") return -11;
              if (u.typeName === "E3") return -10;
              if (u.typeName === "E1") return -9;
            }
          }
          if (eastAfldEverDead && !(padClearedAir && freeAirAnchor)) {
        ...
        const dFree = (u) => (freeAirAnchor
          ? missionEightDistance(freeAirAnchor, u)
          : 0);
        // l519: combat units prefer STRONGER first (l518k pass2 hit E4@10 HP
        // while full hospital pack lived → death cluster@47–48k). Structures
        // still prefer weaker (finish kills).
        const combatAir = new Set([
          "E1", "E2", "E3", "E4", "LTNK", "BGGY", "ARTY", "TRAN",
        ]);
        const strCmp = combatAir.has(a.typeName) && combatAir.has(b.typeName)
          ? b.strength - a.strength
      ...
      if (airTarget) {
        ...
        state.airstrike.orders.push({
          tick: snapshot.tick,
          target: airTarget.typeName,
          cellX: airTarget.cellX,
          cellY: airTarget.cellY,
          targetStrength: airTarget.strength,
        });
        state.airstrike.pending = {
          orderTick: snapshot.tick,
          targetKey: objectKey(airTarget),
          targetType: airTarget.typeName,
          targetStrength: airTarget.strength,
          minStrength: airTarget.strength,
        };
      }
    }
    // AFLD standoff south of east GUNs (y≥12, x≈48–52) — engine near_dist.
    const afldStandoff = { cellX: 50, cellY: 14 };
    const afld = hostiles.find((h) => (
      h.typeName === "AFLD" && h.cellX === 54 && h.cellY === 8 && h.strength > 0
    ));
    const eastProc = hostiles.find((h) => (
      h.typeName === "PROC" && h.cellX >= 40 && h.strength > 0
    ));
    ...
    // Wounded free (str < 80) peel south to village screen — do not die trading.
    // Mid-map only (ridge free stays in BGGY finish).
    // l381: do NOT pull free off residual pad (x≥30, residual commit) — free@
    ...
    for (const tank of freeForce.filter((t) => (
      ...
      state.villageGuardKeys.add(key);
      state.strikeKeys.delete(key);
      queueMissionEightRole(commands, `east-b-post-sam-wounded-village-${key}`,
        [tank], { cellX: 12, cellY: 52 }, MODIFIER_ALT, 15);
    }
    // l259/l274: ridge assault down to str≥5 so near-dead free still finishes BGGY.
    // l381: residual pad free keeps fighting down to str≥5 (All Destr. hunt).
    ...
    const safeStand = { cellX: 40, cellY: 14 };
    if (assaultForce.length === 0 && freeForce.length === 0) {
      // Still run village screen below.
    } else {
      for (const tank of assaultForce) {
        const key = objectKey(tank);
        clearMissionEightUnitRoleKey(key);
        state.strikeKeys.add(key);
        state.baseGuardKeys.delete(key);
        // After all SAMs dead, free pulls WEST immediately (do not wait for
        // A-10 order tick — l209 free bled@39,14 until 40200 then died).
        // l216: free@202@37,15 still dies by 40200 — stop+hard peel first,
        // deeper wait cell (24,26) off corridor GUN/ARTY fire.
        // l488: residual latch MUST run before airDone. TRACE l479 residual@38700
        // with SE SAMs still 400 + free rail east → pad@39270 free 391@38,11.
        // l486 trapped latch inside airDone (needs allSamsDead|A-10|soft AFLD)
        // → residual only@39900 free late@pad trading LTNK freeLast@41400.
        // free-near chips SE SAMs once free holds pad; do not wait allSamsDead.
        const healthyEastFree = tank.strength >= 100
          && tank.cellY <= 25
          && tank.cellX >= 20
          && snapshot.tick >= 38_500
        ...
        // After residual: never south-peel. Rail east to pad if still west —
        // even while SE SAMs live (free-near finishes them on pad).
        if (state.eastBResidualCommit && tank.strength >= 40
          && tank.cellX < 36 && tank.cellY <= 28) {
          const eastRail = { cellX: 40, cellY: 14 };
          // Kite armor east/north of free, not south village.
          const chaseArmor = hostiles.filter((h) => (
            h.strength > 0
            && missionEightDistance(tank, h) <= 3
            && (h.typeName === "LTNK" || h.typeName === "BGGY")
          ...
          if (chaseArmor && tank.strength >= 80) {
            // Fight only d≤1 adj; else step east away.
            if (missionEightDistance(tank, chaseArmor) <= 1) {
              queueMissionEightRole(commands, `east-b-post-sam-east-adj-${key}`,
                [tank], chaseArmor, 0, 1);
              continue;
            }
            const stepEast = {
              cellX: Math.min(42, tank.cellX + 3),
              cellY: Math.max(12, Math.min(16, tank.cellY)),
            };
            queueMissionEightRole(commands, `east-b-post-sam-east-kite-${key}`,
              [tank], stepEast, MODIFIER_ALT, 1);
            continue;
          }
          queueMissionEightRole(commands, `east-b-post-sam-east-rail-${key}`,
            [tank], eastRail, MODIFIER_ALT, 2);
          continue;
        }
        const airDone = state.allSamsDeadTick !== undefined
          || (state.airstrike.orders?.length ?? 0) >= 1
          || (afld && afld.strength < (afld.maxStrength ?? 1000))
          // residual pad path before all SAMs dead (l479 free@pad@39k)
          || Boolean(state.eastBResidualCommit);
        // l251b: hold@11,48 through A-10 pass-3. Proven peel 30,20→18,32→11,48.
        // Closed residual peels: south/west-first/peel-snap/stop-hard/l285–
        // l295 thrash; l292/l293 chip; l296–l297 peel variants. l298/l304:
        // soft-def cadence 3 + softest keeps free@148 hide / @139@53850;
        // wall is civ-9 before pass-3 (not free death).
        const nearSafe = { cellX: 30, cellY: 20 };
        const midSafe = { cellX: 18, cellY: 32 };
        const waitCell = { cellX: 11, cellY: 48 };
        if (airDone) {
          const airReady = Boolean(
            snapshot.sidebar?.entries?.find((e) => (
              e.assetName === "SW_AirStrike" && e.completed
            ))
          );
          const passes = state.airstrike.orders?.length ?? 0;
          // Peel/hold through pass-3 so free parks before re-arm + pass-3 window.
          ...
          // l432c: village peel = LTNK death corridor. Healthy free residual
          // EAST for pad free-near (latch+rail now pre-airDone above).
          if (passes < 3 && !state.eastBResidualCommit) {
            // Kite south-west away from chase armor — never stand-and-trade LTNK
            // mid-peel (l432 free died trading). Only pre-residual.
            const chaseArmor = hostiles.filter((h) => (
              h.strength > 0
              && missionEightDistance(tank, h) <= 4
              && (h.typeName === "LTNK" || h.typeName === "BGGY"
            ...
            if (tank.cellX >= 20 && tank.cellY <= 28) {
              queueMissionEightRole(commands, `east-b-post-sam-peel2-${key}`,
                [tank], midSafe, MODIFIER_ALT, 8);
              continue;
            }
            if (tank.cellY < 46 || tank.cellX > 14) {
              queueMissionEightRole(commands, `east-b-post-sam-peel3-${key}`,
                [tank], waitCell, MODIFIER_ALT, 10);
              continue;
            }
          }
          // Latch ridge arrival for soft-inf window.
          if (tank.cellY >= 46 && tank.cellX <= 16 && tank.strength >= 80) {
            state.eastBFreeRidgeTick ??= snapshot.tick;
          }
          // Residual clear after pass-3. l323 free@139@54300 ord=3 neut=8 then
          // died mid-clear@43,19 / left village → 9th civ. l330: residual left
          // early (commit treated missing AFLD as soft) → civ-9@56790 free@139
          // never engaged (hard base). l331: commit only on real soft/dead AFLD
          // (known cell or discharge chip) OR pass≥4; never !afldLive.
          // l334: require free already in village (y≥44) so soft AFLD after
          // pass-1 cannot divert free off the peel rail mid-map.
          // l341–l348 residual latch. l353: restore pass3-based late residual
          // (early A-10 multi-pass back); engine chips AFLD only when free x≥20.
          {
            const afldEast = hostiles.find((h) => (
              h.typeName === "AFLD" && h.cellX >= 50 && h.cellY <= 12 && h.strength > 0
            )) ?? hostiles.find((h) => (
              h.typeName === "AFLD" && h.cellX === 54 && h.cellY === 8 && h.strength > 0
            ));
            if (afldEast) state.eastBSawEastAfld = true;
            const afldSoft = Boolean(afldEast && afldEast.strength < 700);
            const afldChipped = (state.airstrike.discharges ?? []).some((d) => (
              d.target === "AFLD" && (
                d.targetDamaged
            ...
            if (commitResidual) {
              ...
              // l519w: skip reapproach when free hospital peel is latched —
              // reapproach vs peel thrash stuck free@35,18 freeLast@50k (l519t–v).
              if (eastPadLive && (tank.cellY > 18 || tank.cellX < 36)
                && tank.strength >= 40
                && !state.eastBFreeHospPeel) {
              ...
              // l519w: skip gun-park bounce during hospital peel (free@35,18
              // thrash with eastPadGunsNow y≤20).
              // l519bj: never gun-park when free <150 after residual — free
              // @39,18 lost free-near and bled 66→18 (l519bi). Soft-hold only.
              // l519bj: never gun-park when free <150 after residual — free
              // @39,18 lost free-near and bled 66→18 (l519bi). Soft-hold only.
              // l519cd: restore gun-park gate as l519bp (no mid skip).
              if (eastPadGunsNow && !eastPadProdNow && !blockingCorridorGuns
                && tank.strength >= 150 && tank.cellY <= 20
                && !state.eastBFreeHospPeel) {
              ...
              if (state.eastBEastPadClearedTick !== undefined
                && tank.strength >= 5) {
                // l519aa keeper: free@170@pad soft-hold until pass≥3 tick≥54k.
                // l519bj: pure park when free <160 after 48k — no kite/fire
                // (kite@36,9 / gun-park pulled free south of free-near band).
                // free≥50@54k required for leaveSoftHold residual.
                const softHold = { cellX: 38, cellY: 11 };
                const airPassesNow = state.airstrike.orders?.length ?? 0;
                state.eastBFreeHospPeel = false;
                // l519bm: free@170@pad — leaveSoftHold pass≥3@54k free≥50.
                // l519co: pack-only midIntercept 52–54k (free door hunt).
                // l519ea/eb pass3 HAND CLOSED. l519eg keeper: post-pass3 3.5min
                // rearm + pass4 TRAN hold → lose@62248 (+2.1k vs da@60138).
                // l519ej: second pack mid 59–62k after pass4 (TRAN/E4 door only;
                // l519do broad late door CLOSED@59138 without pass4 support).
                // After 54k pure softHold + WEAP@56k until late mid.
                const midIntercept = airPassesNow >= 2
                  && tank.strength >= 150
                  && snapshot.tick >= 52_000
                  && snapshot.tick < 54_000;
                const lateDoorMid = airPassesNow >= 4
                  && tank.strength >= 160
                  && snapshot.tick >= 59_000
                  && snapshot.tick < 62_000;
                const leaveSoftHold = airPassesNow >= 3
                  && tank.strength >= 50
                  && snapshot.tick >= 54_000
                  && !lateDoorMid;
                if (midIntercept || lateDoorMid) {
                  // Pack-only: early mid TRAN/E4/E3; late mid TRAN/E4 door only.
                  const pack = hostiles.filter((h) => {
                    if (h.strength <= 0) return false;
                    if (lateDoorMid) {
                      return (h.typeName === "TRAN" || h.typeName === "E4")
                        && h.cellY >= 56 && h.cellY <= 62
                        && h.cellX >= 16 && h.cellX <= 28;
                    }
                    return (h.typeName === "TRAN" || h.typeName === "E4"
                      || h.typeName === "E3")
                      && h.cellY >= 44 && h.cellY <= 62
                      && h.cellX >= 12 && h.cellX <= 30;
                  }).toSorted((a, b) => {
                    const rank = (u) => (
                      u.typeName === "TRAN" ? 0
                        : u.typeName === "E4" ? 1
                  ...
                  if (pack && pack.cellX >= 12) {
                    // Abort HAND nest if free walks west of x=12.
                    if (tank.cellX < 12) {
                      queueMissionEightRole(commands,
                        `east-b-post-sam-mid-hand-abort-${key}`,
                        [tank], softHold, MODIFIER_ALT, 1);
                      continue;
                    }
                    // Late mid: abort if free drifts too deep (l519do thrash).
                    if (lateDoorMid && tank.cellY > 50 && tank.cellX < 18) {
                      queueMissionEightRole(commands,
                        `east-b-post-sam-late-mid-abort-${key}`,
                        [tank], softHold, MODIFIER_ALT, 1);
                      continue;
                    }
                    queueMissionEightRole(commands,
                      `east-b-post-sam-mid-hunt-${key}`,
                      [tank], pack, 0, 3);
                    continue;
                  }
                  // No pack: softHold free-near (not midCell thrash).
                  if (missionEightDistance(tank, softHold) > 2) {
                    queueMissionEightRole(commands,
                      `east-b-post-sam-mid-rehold-${key}`,
                      [tank], softHold, MODIFIER_ALT, 2);
                    continue;
                  }
                  queueMissionEightRole(commands,
                    `east-b-post-sam-mid-repark-${key}`,
                    [tank], softHold, MODIFIER_ALT, 4);
                  continue;
                }
                if (!leaveSoftHold) {
                  // Hard snap: free must stay x≥36 y≤14 (free-near load-bearing).
                  if (tank.cellY > 14 || tank.cellX < 36 || tank.cellX > 40) {
                    queueMissionEightRole(commands,
                      `east-b-post-sam-pad-snap-${key}`,
                      [tank], softHold, MODIFIER_ALT, 1);
                    continue;
                  }
                  // Pure park when free damaged — free-near chips armor.
                  if (tank.strength < 160 && snapshot.tick >= 48_000) {
                    if (missionEightDistance(tank, softHold) > 1) {
                      queueMissionEightRole(commands,
                        `east-b-post-sam-pure-hold-${key}`,
                        [tank], softHold, MODIFIER_ALT, 1);
                      continue;
                    }
                    queueMissionEightRole(commands,
                      `east-b-post-sam-pure-park-${key}`,
                      [tank], softHold, MODIFIER_ALT, 3);
                    continue;
                  }
                  // Healthy free: range-fire pad LTNK only from softHold, no melee.
                  const ltnkNear = hostiles.filter((h) => (
                    h.strength > 0
                    && (h.typeName === "LTNK" || h.typeName === "BGGY")
                    && missionEightDistance(tank, h) <= 2
                  )).toSorted((a, b) => (
                    missionEightDistance(tank, a) - missionEightDistance(tank, b)
                    || a.strength - b.strength
                  ))[0];
                  if (ltnkNear && tank.strength >= 220
                    && missionEightDistance(tank, ltnkNear) <= 1) {
                    queueMissionEightRole(commands,
                      `east-b-post-sam-ltnk-adj-${key}`,
                      [tank], ltnkNear, 0, 1);
                    continue;
                  }
                  if (ltnkNear && tank.strength >= 160
                    && missionEightDistance(tank, softHold) <= 2) {
                    queueMissionEightRole(commands,
                      `east-b-post-sam-ltnk-fire-${key}`,
                      [tank], ltnkNear, 0, 2);
                    continue;
                  }
                  if (missionEightDistance(tank, softHold) > 2) {
                    queueMissionEightRole(commands,
                      `east-b-post-sam-postclear-hold-${key}`,
                      [tank], softHold, MODIFIER_ALT, 2);
                    continue;
                  }
                  queueMissionEightRole(commands,
                    `east-b-post-sam-postclear-park-${key}`,
                    [tank], softHold, MODIFIER_ALT, 4);
                  continue;
                }
                // l519da keeper: after leave freeze free-near; ≥56k softHold
                // {38,11}. l519de y=15 park regressed. l519dy mid-screen CLOSED
                // lose@59257 free chipped walking to {30,32}.
                // l519ej keeper: late door mid 59–62k → lose@92490 free@134
                // hostiles~14 minN=8. l519ek residual assault west CLOSED
                // (lose@77179 free left pad → earlier civ wall).
                // l519el: pad-local residual clear ≥65k free≥100 while STAYING
                // free-near (x≥36 y≤16) — range-fire only d≤6 from free/softHold.
                // ek@65k / em west@80k CLOSED (earlier civ wall).
                // l519eo/eq/er: residual north → lose@92735 free drifted east@44
                // while GUN@16/LTNK@12 uncleared; door TRAN A-10 undamaged.
                // l519es: TRAN A-10 force-chip CLOSED los3-hosp@77739.
                // l519eu: west corridor + residual → lose@92794 free softHold
                // (door wave unlatched residual → snap off west nest).
                // l519ev: LATCH north residual once open; free stays on west
                // nest until clear or free scrap. West gate cadence 1.
                const handLive = hostiles.some((h) => (
                ...
                if (airPassesNow >= 8
                  ...
                  && !handLive
                  && hostilesCount <= 16) {
                  state.eastBNorthResidual = true;
                }
                const residualNorth = Boolean(state.eastBNorthResidual)
                  && tank.strength >= 80
                  && snapshot.tick >= 84_000
                  && !handLive;
                if (residualNorth) {
                  ...
                  if (missionEightDistance(tank, softHold) > 1) {
                    queueMissionEightRole(commands,
                      `east-b-post-sam-north-park-hold-${key}`,
                      [tank], softHold, MODIFIER_ALT, 2);
                  } else {
                    queueMissionEightRole(commands,
                      `east-b-post-sam-north-park-${key}`,
                      [tank], softHold, MODIFIER_ALT, 4);
                  }
                  continue;
                }
                if (!residualNorth
                  && (tank.cellY > 18 || tank.cellX < 34 || tank.cellX > 44)) {
                  queueMissionEightRole(commands,
                    `east-b-post-sam-leave-snap-${key}`,
                    [tank], softHold, MODIFIER_ALT, 2);
                  continue;
                }
                const padLocalResidual = airPassesNow >= 5
                  && tank.strength >= 100
                  && snapshot.tick >= 65_000
                  && tank.cellX >= 36
                  && tank.cellY <= 16
                  && !residualNorth;
                if (padLocalResidual) {
                  const localPrey = hostiles.filter((h) => (
                    h.strength > 0
                    && h.type !== 4 // no structure walk-melee; free-near + A-10
                    && !["C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8", "C9", "C10"]
                      .includes(h.typeName)
                    && (missionEightDistance(tank, h) <= 6
                      || missionEightDistance(softHold, h) <= 6)
                    && h.cellX >= 30
                    && h.cellY <= 22
                  )).toSorted((a, b) => {
                    const rank = (u) => (
                      u.typeName === "LTNK" || u.typeName === "BGGY" ? 0
                        : u.typeName === "ARTY" ? 1
                          : u.typeName === "E4" ? 2
                            : u.typeName === "E3" || u.typeName === "E1" ? 3
                              : u.typeName === "TRAN" ? 4
                                : 5
                    );
                    return rank(a) - rank(b)
                  ...
                  if (localPrey
                    && tank.cellX >= 36 && tank.cellY <= 16
                    && missionEightDistance(tank, softHold) <= 4) {
                    if (missionEightDistance(tank, localPrey) <= 2
                      && tank.strength >= 120) {
                      queueMissionEightRole(commands,
                        `east-b-post-sam-pad-local-fire-${key}`,
                        [tank], localPrey, 0, 2);
                      continue;
                    }
                    const step = {
                      cellX: Math.min(42, Math.max(36, localPrey.cellX)),
                      cellY: Math.min(16, Math.max(11, localPrey.cellY)),
                    };
                    if (missionEightDistance(tank, step) > 1) {
                      queueMissionEightRole(commands,
                        `east-b-post-sam-pad-local-step-${key}`,
                        [tank], step, MODIFIER_ALT, 3);
                      continue;
                    }
                  }
                }
                const holdCell = snapshot.tick >= 56_000
                  ? softHold
                  : {
                    cellX: Math.min(42, Math.max(36, tank.cellX)),
                    cellY: Math.min(16, Math.max(11, tank.cellY)),
                  };
                if (missionEightDistance(tank, holdCell) > 1) {
                  queueMissionEightRole(commands,
                    `east-b-post-sam-leave-hold-${key}`,
                    [tank], holdCell, MODIFIER_ALT, 4);
                  continue;
                }
                queueMissionEightRole(commands,
                  `east-b-post-sam-leave-park-${key}`,
                  [tank], holdCell, MODIFIER_ALT, 8);
                continue;
              }
              ...
              // l514: exclude y≥45 combat units from residual hunt (free pathing
              // south killed C5:22@50910). Structures any y; units only y≤40.
              const eastStructs = hostiles.filter((h) => (
                h.strength > 0
                && (h.typeName === "AFLD" || h.typeName === "PROC"
                  || h.typeName === "GUN" || h.typeName === "HQ"
                  || h.typeName === "HAND" || h.typeName === "FACT"
                  || h.typeName === "NUKE" || h.typeName === "WEAP"
                  || h.typeName === "PYLE" || h.typeName === "HPAD"
                  || h.typeName === "SAM")
              ));
              const eastPrey = (eastStructs.length > 0 ? eastStructs : hostiles.filter((h) => (
                h.strength > 0
                && h.type !== 4
                && h.cellY <= 40
                && !["C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8", "C9", "C10"].includes(h.typeName)
              ))).toSorted((a, b) => {
                // l363: GUN corridor first (41,8 / 42,5). l358 AFLD-first left free
                // thrashing 36–39,y15–17 under GUN fire never reaching AFLD@54,8.
                // Kill blocking turrets, then AFLD (A-10 already chipped free_mid).
                const rank = (u) => {
                  if (u.typeName === "GUN" && u.cellX === 41 && u.cellY === 8) return 0;
                  if (u.typeName === "GUN" && u.cellX === 42 && u.cellY === 5) return 1;
                  // l366: GUN@45,16 blocks AFLD path (free@37,14 thrash).
                  if (u.typeName === "GUN" && u.cellX === 45 && u.cellY === 16) return 2;
                  // l416: AFLD/PROC before GUN@50,16 (was rank 3 via x>=38).
                  if (u.typeName === "AFLD") return 3;
                  if (u.typeName === "PROC") return 4;
                  if (u.typeName === "GUN" && u.cellX >= 38 && u.cellY <= 16) return 5;
                  if (u.typeName === "HAND") return 6;
                  if (u.typeName === "FACT") return 7;
                  if (u.typeName === "NUKE" || u.typeName === "HQ") return 8;
                  if (u.typeName === "GUN") return 9;
                  if (u.typeName === "LTNK" || u.typeName === "ARTY") return 10;
                  return 11;
                };
                return rank(a) - rank(b)
                  || a.strength - b.strength
              ...
              if (eastPrey) {
                // l407/l408/l415: corridor GUN first always. Match gun-dive filter
                // (exclude GUN@50,16 — deferred until pad production dead).
                const corridorGunsLive = hostiles.some((h) => (
                  h.typeName === "GUN" && h.strength > 0
                  && ((h.cellX === 41 && h.cellY === 8)
                    || (h.cellX === 42 && h.cellY === 5)
                    || (h.cellX === 45 && h.cellY === 16)
                    || (h.cellX >= 38 && h.cellX < 50 && h.cellY <= 16))
                ));
                const gunRank = (u) => {
                  if (u.cellX === 41 && u.cellY === 8) return 0;
                  if (u.cellX === 42 && u.cellY === 5) return 1;
                  if (u.cellX === 45 && u.cellY === 16) return 2;
                  if (u.cellX >= 38 && u.cellX < 50 && u.cellY <= 16) return 3;
                  return 9;
                };
                const nearCorridorGun = hostiles.filter((h) => (
                  h.typeName === "GUN" && h.strength > 0
                  && ((h.cellX === 41 && h.cellY === 8)
                    || (h.cellX === 42 && h.cellY === 5)
                    || (h.cellX === 45 && h.cellY === 16)
                ...
                // l418: HIGH-HP pad dive FIRST after corridor guns — free@39,14
                ...
                const nearBggy = hostiles.filter((h) => (
                  h.typeName === "BGGY" && h.strength > 0
                  && missionEightDistance(tank, h) <= 5
                )).toSorted((a, b) => (
                  missionEightDistance(tank, a) - missionEightDistance(tank, b)
                ))[0];
                if (nearBggy && tank.strength >= 20) {
                  queueMissionEightRole(commands, `east-b-post-sam-bggy-${key}`,
                    [tank], nearBggy, 0, 1);
                  continue;
                }
                if (nearCorridorGun && tank.cellX >= 32 && tank.strength >= 20
                  && missionEightDistance(tank, nearCorridorGun) <= 18) {
                  // Approach hop west of GUN@41,8 / south of GUN@42,5 so free
                  // does not path SE into BGGY@49,14 (l408 free@48,14 scrap).
                  let approach = null;
                  if (nearCorridorGun.cellX === 41 && nearCorridorGun.cellY === 8) {
                    approach = { cellX: 38, cellY: 12 };
                  } else if (nearCorridorGun.cellX === 42 && nearCorridorGun.cellY === 5) {
                    approach = { cellX: 39, cellY: 10 };
                  }
                  if (approach
                    && missionEightDistance(tank, nearCorridorGun) > 4
                    && missionEightDistance(tank, approach) > 2
                    && tank.cellX < nearCorridorGun.cellX + 1) {
                    queueMissionEightRole(commands, `east-b-post-sam-gun-approach-${key}`,
                      [tank], approach, MODIFIER_ALT, 1);
                    continue;
                  }
                  queueMissionEightRole(commands, `east-b-post-sam-gun-dive-${key}`,
                    [tank], nearCorridorGun, 0, 1);
                  continue;
                }
                // l405/l407: only clear infantry at d≤2 when no corridor GUN in
                // fire band (GUN kills free faster than rifles).
                const nearInf = hostiles.filter((h) => (
                  (h.typeName === "E1" || h.typeName === "E2" || h.typeName === "E3"
                    || h.typeName === "E4")
                  && h.strength > 0
                  && missionEightDistance(tank, h) <= 2
                )).toSorted((a, b) => a.strength - b.strength)[0];
                if (nearInf && tank.strength >= 60 && !nearCorridorGun) {
                  queueMissionEightRole(commands, `east-b-post-sam-inf-clear-${key}`,
                    [tank], nearInf, 0, 1);
                  continue;
                }
                // l372/l376: hard attack-move pad structures once free x≥36.
                if (tank.cellX >= 36
                  && (eastPrey.typeName === "PROC" || eastPrey.typeName === "HAND"
                    || eastPrey.typeName === "FACT" || eastPrey.typeName === "NUKE"
                    || eastPrey.typeName === "HQ" || eastPrey.typeName === "AFLD"
                    || eastPrey.typeName === "GUN" || eastPrey.typeName === "WEAP"
                    || eastPrey.typeName === "PYLE")) {
                  queueMissionEightRole(commands, `east-b-post-sam-struct-dive-${key}`,
                    [tank], eastPrey, 0, 1);
                  continue;
                }
                // Hunt units after structures cleared (All Destr. BadGuy win).
                // l386: free@28,14 thrash with hostiles=11 d>10 — no range cap.
                if (eastStructs.length === 0 && eastPrey) {
                  queueMissionEightRole(commands, `east-b-post-sam-unit-hunt-${key}`,
                    [tank], eastPrey, 0, 1);
                  continue;
                }
                // Reverse peel rail. l349: picking first cell with d>2 always
                // re-targeted 18,32 after free had already passed it (bounce).
                // Advance to the furthest rail cell free is still "behind".
                // l363: +GUN approach hop {40,12} so free reaches turret fire band
                // before AFLD dive (was stuck@39,16 under corridor guns).
                const rail = [
                  { cellX: 18, cellY: 32 },
                  { cellX: 30, cellY: 20 },
                  { cellX: 36, cellY: 16 },
                ...
                let nextHop = rail[rail.length - 1];
                for (let i = 0; i < rail.length; i++) {
                  const cell = rail[i];
                  const behind = tank.cellX < cell.cellX - 1
                    || tank.cellY > cell.cellY + 1;
                  if (behind && missionEightDistance(tank, cell) > 1) {
                    nextHop = cell;
                    break;
                  }
                }
                // l367: after corridor GUNs dead, do NOT snap free back to 36,16
                // (free@39 thrash under remap while AFLD still live). Skip rail
                // once free past 36,16 or guns clear — go straight to prey.
                // l371: free@32,18 was re-railed while GUN@41,8 d=10 — skip rail
                // once free x≥30 so gun-clear / AFLD dive can run.
                const pastRail = tank.cellX >= 30 && tank.cellY <= 20;
                if (missionEightDistance(tank, nextHop) > 2
                  && !pastRail
                  && corridorGunsLive
                  && !(eastPrey.typeName === "GUN" && missionEightDistance(tank, eastPrey) <= 12)) {
                  queueMissionEightRole(commands, `east-b-post-sam-rail-${key}`,
                    [tank], nextHop, MODIFIER_ALT, 1);
                  continue;
                }
                // l407: stage-hold CLOSED on residual — l406 free@str52 retreated
                // to 31,26 and died. Press GUN/AFLD or die fighting, no west snap.
                // l363: engage corridor GUN first (high cadence attack-move).
                if (eastPrey.typeName === "GUN" && tank.cellX >= 30) {
                  queueMissionEightRole(commands, `east-b-post-sam-gun-clear-${key}`,
                    [tank], eastPrey, 0, 1);
                  continue;
                }
                // l361/l363: AFLD dive only after corridor GUNs dead (or none left).
                if (eastPrey.typeName === "AFLD" && tank.cellX >= 30 && !corridorGunsLive) {
                  queueMissionEightRole(commands, `east-b-post-sam-afld-dive-${key}`,
                    [tank], eastPrey, 0, 1);
                  continue;
                }
                if (missionEightDistance(tank, eastPrey) <= 18) {
                  queueMissionEightRole(commands, `east-b-post-sam-clear-${key}`,
                    [tank], eastPrey, 0, 1);
                  continue;
                }
                queueMissionEightRole(commands, `east-b-post-sam-clear-rail-${key}`,
                  [tank], {
                    cellX: Math.max(eastPrey.cellX - 4, 28),
                    cellY: Math.min(Math.max(eastPrey.cellY + 4, 14), 22),
                  }, MODIFIER_ALT, 1);
                continue;
              }
            }
          }
          // l259/l274 BGGY micro at hide {5,48}. Residual class (not peel/SE/NE).
          // Closed residual: l275 close-step, l276 hide{5,50}, l280 1v1-close,
          // l283/c E4 west-peel, l284 E4-crush (=l259 wall; crush fires too late).
          // Keep str≥5 floor + finish≤90. Death wall: free@76 after BGGY#1,
          ...
          if (villageSoftDef) {
            ...
            if (tank.strength >= 130) {
              ...
              const phase = Math.floor(snapshot.tick / 120) % 2;
              const post = phase === 0 ? bait : hideCell;
              if (missionEightDistance(tank, post) > 1) {
                queueMissionEightRole(commands, `east-b-post-sam-bait-${key}`,
                  [tank], post, MODIFIER_ALT, 6);
                continue;
              }
            }
            if (missionEightDistance(tank, hideCell) > 1 && tank.strength >= 40
              && tank.strength < 130) {
              queueMissionEightRole(commands, `east-b-post-sam-timed-hide-${key}`,
            ...
            // l274: finish ≤90. l291b: finish ≤110 when free soft (#8@94).
            // l292: ALWAYS cadence 1 on soft/finish BGGY.
            // l294c sticky focus kept (no-op vs control death wall).
            const liveBggyByKey = new Map(
              nearBuggies.map((h) => [objectKey(h), h]),
            );
            if (state.eastBBggyFocusKey
              && !liveBggyByKey.has(state.eastBBggyFocusKey)) {
              state.eastBBggyFocusKey = undefined;
            }
            const pickStickyBggy = (candidates) => {
              if (!candidates.length) return undefined;
              if (state.eastBBggyFocusKey) {
                const held = candidates.find(
                  (h) => objectKey(h) === state.eastBBggyFocusKey,
                );
                if (held) return held;
              }
              const picked = candidates.toSorted((a, b) => (
                a.strength - b.strength
                || missionEightDistance(tank, a) - missionEightDistance(tank, b)
              ))[0];
              if (picked) state.eastBBggyFocusKey = objectKey(picked);
              return picked;
            };
            const finishCap = tank.strength < 100 ? 110 : 90;
            const finishBuggy = pickStickyBggy(nearBuggies.filter((h) => (
              h.strength <= finishCap
              && missionEightDistance(tank, h) <= 5
            )));
            if (finishBuggy) {
              queueMissionEightRole(commands, `east-b-post-sam-finish-bggy-${key}`,
                [tank], finishBuggy, 0, bggyCadence);
              continue;
            }
            const healthyBuggies = nearBuggies.filter((h) => h.strength > 50);
            const meleeHealthy = healthyBuggies.filter((h) => (
              missionEightDistance(tank, h) <= 5
            ));
            // Soft press at d≤8 — sticky softest ≤100.
            const softTarget = pickStickyBggy(nearBuggies.filter((h) => (
              h.strength > 0 && h.strength <= 100
              && missionEightDistance(tank, h) <= 8
            )));
            if (softTarget && tank.strength >= 5) {
              queueMissionEightRole(commands, `east-b-post-sam-press-soft-${key}`,
                [tank], softTarget, 0, bggyCadence);
              continue;
            }
            if (meleeHealthy.length >= 2) {
              // l294b/c: focus softest (sticky) when free≥60; hold only scrap.
              if (tank.strength >= 60) {
                const focus = pickStickyBggy(meleeHealthy);
                queueMissionEightRole(commands, `east-b-post-sam-focus-bggy-${key}`,
                  [tank], focus, 0, bggyCadence);
                continue;
              }
              queueMissionEightRole(commands, `east-b-post-sam-2v1-hold-${key}`,
                [tank], hideCell, MODIFIER_ALT, 8);
              continue;
            }
            // l280 close-step closed (free residual@#1 worse). l259 1v1 at d≤8.
            if (healthyBuggies.length === 1 && tank.strength >= 100
              && missionEightDistance(tank, healthyBuggies[0]) <= 8) {
              const one = pickStickyBggy(healthyBuggies);
              queueMissionEightRole(commands, `east-b-post-sam-1v1-bggy-${key}`,
                [tank], one, 0, bggyCadence);
              continue;
            }
            if (meleeHealthy.length === 1 && tank.strength >= 5) {
              // l292: when E4 swarm d≤5 and free soft but BGGY still >50, step
              // one cell south (keep d≤5 to BGGY) so flamers waste path time —
              // not west peel (l283c stopped #8 DPS). Re-press next tick.
              // l294 E4-int-before-finish closed (regress bk=3).
              const focusOne = pickStickyBggy(meleeHealthy);
              const e4Near = hostiles.filter((h) => (
                h.typeName === "E4" && h.strength > 0
                && missionEightDistance(tank, h) <= 5
              )).length;
              if (e4Near >= 2 && tank.strength < 80 && tank.strength >= 30
                && focusOne.strength > 50
                && tank.cellY < 52) {
                const step = {
                  cellX: tank.cellX,
                  cellY: Math.min(52, tank.cellY + 1),
                };
                if (missionEightDistance(step, focusOne) <= 5) {
                  queueMissionEightRole(commands, `east-b-post-sam-e4-south-${key}`,
                    [tank], step, MODIFIER_ALT, 2);
                  continue;
                }
              }
              queueMissionEightRole(commands, `east-b-post-sam-press-bggy-${key}`,
                [tank], focusOne, 0, bggyCadence);
              continue;
            }
            if (nearBuggies.length === 0) state.eastBBggyFocusKey = undefined;
            const nearArmor = hostiles.filter((h) => (
              h.type === 2 && h.typeName !== "BGGY" && h.strength > 0
              && missionEightDistance(tank, h) <= 2
            )).toSorted((a, b) => (
              missionEightDistance(tank, a) - missionEightDistance(tank, b)
            ))[0];
            if (nearArmor && tank.strength >= 80) {
              queueMissionEightRole(commands, `east-b-post-sam-hide-snap-${key}`,
                [tank], nearArmor, 0, 8);
              continue;
            }
            queueMissionEightRole(commands, `east-b-post-sam-hide-hold-${key}`,
              [tank], hideCell, MODIFIER_ALT, 12);
            continue;
          }

          if (missionEightDistance(tank, waitCell) > 3) {
            queueMissionEightRole(commands, `east-b-post-sam-repark-${key}`,
              [tank], waitCell, MODIFIER_ALT, 4);
            continue;
          }
          // Soft-def: softest-first d≤5 cad3 keeps free@148 hide (l298).
          // Distance-first/melee-only closed as control death wall / early die.
          if (tank.strength >= 120) {
            const softThreat = hostiles.filter((h) => (
              h.type === 1 && h.strength > 0
              && missionEightDistance(waitCell, h) <= 5
            )).toSorted((a, b) => (
              a.strength - b.strength
              || missionEightDistance(waitCell, a) - missionEightDistance(waitCell, b)
            ))[0];
            if (softThreat) {
              queueMissionEightRole(commands, `east-b-post-sam-soft-def-${key}`,
                [tank], softThreat, 0, 3);
              continue;
            }
          }
          queueMissionEightRole(commands, `east-b-post-sam-wait-a10-${key}`,
            [tank], waitCell, MODIFIER_ALT, 4);
          continue;
        }
        // Pre-A-10: hold safe stand on corridor (y 12–16, x≤42).
        // (Stand-slip closed as no-op: this block only runs after all SAMs dead;
        // 381→306 damage is during SE-SAM rail, not here.)
        if (tank.cellX > 42 || tank.cellY > 16 || tank.cellY < 12) {
          queueMissionEightRole(commands, `east-b-post-sam-east-${key}`,
            [tank], safeStand, MODIFIER_ALT, 4);
          continue;
        }
        if (tank.cellX < 36 || Math.abs(tank.cellY - 14) > 1) {
          queueMissionEightRole(commands, `east-b-post-sam-east-${key}`,
            [tank], safeStand, MODIFIER_ALT, 6);
          continue;
        }
        if (afld && afld.strength > 0) {
          queueMissionEightRole(commands, `east-b-post-sam-afld-hold-${key}`,
            [tank], safeStand, MODIFIER_ALT, 8);
          continue;
        }
        queueMissionEightRole(commands, `east-b-post-sam-wait-a10-${key}`,
          [tank], waitCell, MODIFIER_ALT, 12);
      }
    }
    // Village screen — path returns true (skips main Moebius). Restore l519aa
    // half-screen (l519ac/ad TRAN-priority absorb regressed civ@40k; l519af
    // bodyMin=3 regressed civ@42k maxAir=1). Only TRAN/E4 re-rank within
    // nearest-anchor threats. (Active when residual free path does not return.)
    const villageAnchors = [
      { cellX: 8, cellY: 57 }, { cellX: 3, cellY: 60 }, { cellX: 6, cellY: 60 },
      { cellX: 12, cellY: 50 }, { cellX: 20, cellY: 48 }, { cellX: 4, cellY: 55 },
      { cellX: 16, cellY: 52 }, { cellX: 10, cellY: 54 },
    ];
    const villageThreats = hostiles.filter((h) => (
      (h.type === 1 || h.type === 2 || h.typeName === "TRAN")
      && h.strength > 0
      && villageAnchors.some((a) => missionEightDistance(h, a) <= 18)
    )).toSorted((a, b) => {
      const rank = (h) => (
        h.typeName === "TRAN" ? 0 : h.typeName === "E4" ? 1 : 2
      );
      return rank(a) - rank(b)
        || Math.min(...villageAnchors.map((an) => missionEightDistance(a, an)))
          - Math.min(...villageAnchors.map((an) => missionEightDistance(b, an)))
        || a.strength - b.strength;
    });
    const villageInf = attackers.filter((u) => (
      u.type === 1 && u.strength > 0
    ...
    for (const inf of villageInf) {
      if (!state.villageGuardKeys.has(objectKey(inf))) {
        state.villageGuardKeys.add(objectKey(inf));
      }
    }
    const villageTanks = attackers.filter((u) => (
      u.typeName === "MTNK" && state.villageGuardKeys.has(objectKey(u))
      && u.strength >= 40
      && !state.eastBPostWestProducedTankKeys.has(objectKey(u))
    ));
    const screen = [...villageInf, ...villageTanks];
    // l519aa half-screen: ceil(n/2) hunters, remainder Moebius body.
    if (villageThreats.length > 0 && screen.length > 0) {
      const threat = villageThreats[0];
      const hunterN = Math.max(1, Math.ceil(screen.length / 2));
      const hunters = screen.filter((u) => u.strength >= 15).slice(0, hunterN);
      const holders = screen.filter((u) => !hunters.includes(u));
      if (hunters.length > 0) {
        queueMissionEightRole(commands, "east-b-post-sam-village-screen",
          hunters, threat, 0, 8);
      }
      if (holders.length > 0) {
        queueMissionEightRole(commands, "east-b-post-sam-village-hold",
          holders, { cellX: 6, cellY: 58 }, MODIFIER_ALT, 10);
      }
    } else if (screen.length > 0) {
      queueMissionEightRole(commands, "east-b-post-sam-village-hold",
        screen, { cellX: 6, cellY: 58 }, MODIFIER_ALT, 12);
    }
    return true;
  }
  if (tanks.length === 0) return false;
  ...
  // l102/l179: latch first GUN kill so commit/rail ignore respawn.
  // TRACE l179: free frozen@17,20 gun-to-fire-cell while GUN repaired 160→400
  // because latch required free on spine AND GUN currently 0 same tick — GUN
  // respawned between observations. Latch on first observed kill/near-kill.
  const westernGunObj = hostiles.find((hostile) => (
    hostile.typeName === "GUN" && hostile.cellX === 11 && hostile.cellY === 18
  ));
  const westernGunLiveNow = Boolean(westernGunObj && westernGunObj.strength > 0);
  if (state.eastBPostWestGunClearedTick === undefined) {
    if (!westernGunLiveNow) {
      // GUN absent or strength 0 — first kill.
      state.eastBPostWestGunClearedTick = snapshot.tick;
    } else if ((westernGunObj.strength ?? 400) <= 30
      && tanks.some((tank) => (
        state.eastBPostWestProducedTankKeys.has(objectKey(tank))
        && tank.cellY <= 28 && tank.cellX <= 22 && tank.strength >= 40
      ))) {
      // Near-dead with free in theatre — treat as cleared (finish will complete).
      state.eastBPostWestGunClearedTick = snapshot.tick;
    }
  }
  ...
  // Advance force-move stages when tanks are already past the waypoint.
  if (waypoint?.forceMove && state.routeStage < route.length) {
    const past = tanks.filter((tank) => (
      tank.cellY <= waypoint.cellY + 2
      && Math.abs(tank.cellX - waypoint.cellX) <= 8
    )).length;
    if (past >= Math.max(1, Math.ceil(tanks.length * 0.5))) {
      state.routeProgress.push({
        tick: snapshot.tick,
        stage: state.routeStage,
        label: waypoint.label,
        ...
        cellY: waypoint.cellY,
        arrivals: past,
        reason: "post-west-force-move",
      });
      state.routeStage += 1;
      state.routeStageStartedTick = snapshot.tick;
    }
  }
  // Advance SAM stages when the targeted SAM is dead.
  if (waypoint?.typeName === "SAM") {
    const tx = waypoint.targetCellX ?? waypoint.cellX;
    const ty = waypoint.targetCellY ?? waypoint.cellY;
    const stillUp = hostiles.some((hostile) => (
      hostile.typeName === "SAM" && hostile.cellX === tx && hostile.cellY === ty
      && hostile.strength > 0
    ));
    if (!stillUp && state.routeStage < route.length) {
      ...
      state.routeProgress.push({
        ...
        cellX: tx,
        cellY: ty,
        arrivals: tanks.length,
        reason: "post-west-sam-dead",
      });
      state.routeStage += 1;
      state.routeStageStartedTick = snapshot.tick;
    }
  ...
  // freeLeader = northernmost free (proven tip for western GUN + NE/NW/SE).
  // l290b strongest freeLeader / l290d soft-park closed (allSams never / peel thrash).
  // l289–l289i partner-reserve closed.
  ...
  for (const tank of tanks) {
    ...
    if (prev) {
      ...
      if (noProgress && sameBand && snapshot.tick - prev.since >= 150) {
        stuck = true;
      }
      if (!noProgress) {
        stuckMap.set(tankKey, {
    ...
    // l176k: free#2 SE hunter post-GUN only (not freeLeader). During-GUN SE
    // closed (l267b). l269 SE hard-march closed. l279 BGGY-reserve closed.
    // l290–e NE 2v1/soft-peel/soft-park closed: freeLeader residual worse or
    // allSams never; control path is freeLeader alone@NE then sole SE.
    // Keep denser NE rockets only (shorter NE kill window).
    const freeLeaderKey = freeLeader ? objectKey(freeLeader) : null;
    const seSamsLive = hostiles.filter((hostile) => (
      hostile.typeName === "SAM" && hostile.strength > 0
      && ((hostile.cellX === 43 && hostile.cellY === 14)
        || (hostile.cellX === 52 && hostile.cellY === 14)
        || (hostile.cellX === 54 && hostile.cellY === 5))
    ));
    const seHunter = !westernGun
      && !isScrap
      && freePostWestCount >= 2
      && state.eastBPostWestProducedTankKeys.has(tankKey)
      && tankKey !== freeLeaderKey
      && tank.strength >= 40
      && seSamsLive.length > 0;
    let railTarget = target;
    if (seHunter) {
      railTarget = seSamsLive.toSorted((left, right) => (
        missionEightDistance(tank, left) - missionEightDistance(tank, right)
        || left.cellX - right.cellX
        || left.id - right.id
      ))[0];
    }
    const rail = eastBPostWestRailApproach(tank, railTarget, westernGun, {
      soleFree, stuck, isScrap, freeLeader, waitPartner,
      freeNorthCount, neGun: neGun ?? null, seHunter,
    });
    // TRACE l101/l140: stop combat before post-GUN spine force-move.
    // l176i: stop+force-move same order cycle left free frozen ~60t at 20,13 —
    // stop only on first SE peel cycle, move next cycle (TICKS_PER_ORDER=30).
    // l179c: same freeze at free@17,20 gun-to-fire-cell (stopFirst + move same
    // cycle) — free never joined GUN 2v1; GUN repaired; civ-nine. Stop-only
    // first cycle for ALL stopFirst reasons, not only SE peel.
    const sePeel = rail.reason === "se-sam-east"
      || rail.reason === "se-sam-attack-move"
      || rail.reason === "se-sam-range"
      || rail.reason === "se-sam-peel-west";
    state.eastBPostWestSePeelStopKeys ??= new Set();
    if (rail.stopFirst) {
      if (sePeel && !state.eastBPostWestSePeelStopKeys.has(tankKey)) {
        queueMissionEightStop(commands, `east-b-post-west-se-stop-${tankKey}`, [tank], 1);
        state.eastBPostWestSePeelStopKeys.add(tankKey);
        // Do not queue move same cycle — stop would cancel the path.
        continue;
      } else if (!sePeel && !state.eastBPostWestSpineStopKeys.has(tankKey)) {
        queueMissionEightStop(commands, `east-b-post-west-stop-${tankKey}`, [tank], 1);
        state.eastBPostWestSpineStopKeys.add(tankKey);
        // l179c: same stop-only first cycle for gun-to-fire-cell / spine.
        continue;
      }
    }
    if (rail.engage) {
      ...
      const engageTarget = isWestGun ? westernGun : isNeGun ? neGun : railTarget;
      // l186/l187: SE engage must not path free past x=42 (east GUNs kill free).
      const seEngage = !isWestGun && !isNeGun && eastBIsSeRemainingSam(engageTarget);
      // Hard-cap for SE@43/52 corridor; SE@54 stand may use x up to 44.
      const seHardCap = engageTarget.cellY <= 8 ? 46 : 42;
      if (seEngage && tank.cellX > seHardCap) {
        if (!state.eastBPostWestSePeelStopKeys.has(`${tankKey}:se-pull`)) {
          queueMissionEightStop(commands, `east-b-post-west-se-pull-${tankKey}`, [tank], 1);
          state.eastBPostWestSePeelStopKeys.add(`${tankKey}:se-pull`);
          continue;
        }
        const pull = engageTarget.cellY <= 8
          ? { cellX: 42, cellY: 11 }
          : { cellX: 40, cellY: 14 };
        queueMissionEightRole(commands, `east-b-post-west-rail-${tankKey}`,
          [tank], pull, MODIFIER_ALT, 2);
        continue;
      }
      ...
      // SE: force-move to stand when free is still west of theatre.
      if (seEngage && tank.cellX < 36) {
        queueMissionEightRole(commands, `east-b-post-west-rail-${tankKey}`,
          [tank], { cellX: 40, cellY: 14 }, MODIFIER_ALT, rail.cadence);
        continue;
      }
      queueMissionEightRole(commands, role, [tank], engageTarget, 0, rail.cadence);
      continue;
    }
    // Clamp SE force-move: corridor SAMs x≤42; northern SE@54 may use x≤44.
    let destX = rail.cellX;
    let destY = rail.cellY;
    const seNorthDest = rail.reason.startsWith("se-sam") && destY <= 12;
    const seCap = seNorthDest ? 44 : 42;
    if (rail.reason.startsWith("se-sam") && destX > seCap) {
      destX = seCap;
    }
    // Pull free already past cap (path overshoot) back west with stop.
    if (rail.reason.startsWith("se-sam") && tank.cellX > seCap) {
      if (!state.eastBPostWestSePeelStopKeys.has(`${tankKey}:se-pull`)) {
        queueMissionEightStop(commands, `east-b-post-west-se-pull-${tankKey}`, [tank], 1);
        state.eastBPostWestSePeelStopKeys.add(`${tankKey}:se-pull`);
        continue;
      }
      destX = seNorthDest ? 42 : 40;
      destY = seNorthDest ? 11 : 14;
    }
    queueMissionEightRole(commands, `east-b-post-west-rail-${tankKey}`,
      [tank], { cellX: destX, cellY: destY }, MODIFIER_ALT, rail.cadence);
  }
  ...
  // l174/l290: denser NE GUN rocket assist — free@~242 chips alone then dies;
  // freeTrading earlier (y≤24) + up to 8 E3 + cadence 2 so kill window closes
  // before freeLeader bleeds 259→102 (TRACE control@38700).
  if (!westernGun && neGun) {
    const freeTradingNe = tanks.some((tank) => (
      state.eastBPostWestProducedTankKeys.has(objectKey(tank))
      && tank.cellY <= 24 && tank.cellX <= 28 && tank.cellX >= 12
      && tank.strength >= 40
    ));
    if (freeTradingNe) {
      const rockets = attackers.filter((unit) => (
      ...
      if (rockets.length > 0) {
        const near = rockets.filter((r) => missionEightDistance(r, neGun) <= 8);
        const far = rockets.filter((r) => missionEightDistance(r, neGun) > 8);
        if (near.length > 0) {
        ...
        for (const r of far) {
          queueMissionEightRole(commands, `east-b-post-west-negun-rocket-rail-${objectKey(r)}`,
            [r], { cellX: 20, cellY: 16 }, MODIFIER_ALT, 8);
        }
      }
    }
  }
  ...
  // l178h/l206: village screen throughout SE SAM push (not only after allSams).
  // Only order units already in villageGuardKeys — do not steal free armor.
  // Split: engage nearest pad threat + hold hospital.
  if (state.samDeathTicks.has("13:16") || state.allSamsDeadTick !== undefined) {
    const hospHold = { cellX: 5, cellY: 58 };
    const midHold = { cellX: 12, cellY: 52 };
    const screen = attackers.filter((u) => (
      u.strength > 0 && state.villageGuardKeys.has(objectKey(u))
    ...
    if (screen.length > 0) {
      const nearHosp = hostiles.filter((h) => (
        (h.type === 1 || h.type === 2)
        && missionEightDistance(h, hospHold) <= 14
      )).toSorted((a, b) => (
        missionEightDistance(a, hospHold) - missionEightDistance(b, hospHold)
        || a.strength - b.strength
      ))[0];
      if (nearHosp) {
        const hunters = screen.filter((u) => u.strength >= 20).slice(0, 8);
        const holders = screen.filter((u) => !hunters.includes(u));
        if (hunters.length > 0) {
          queueMissionEightRole(commands, "east-b-se-village-screen",
            hunters, nearHosp, 0, 8);
        }
        if (holders.length > 0) {
          queueMissionEightRole(commands, "east-b-se-village-hold",
            holders, hospHold, MODIFIER_ALT, 15);
        }
      } else {
        // Fan out: half hospital, half mid village intercept.
        const half = Math.max(1, Math.ceil(screen.length / 2));
        queueMissionEightRole(commands, "east-b-se-village-hold",
          screen.slice(0, half), hospHold, MODIFIER_ALT, 15);
        if (screen.length > half) {
          queueMissionEightRole(commands, "east-b-se-village-mid",
            screen.slice(half), midHold, MODIFIER_ALT, 18);
        }
      }
    }
  }
...
function eastBSamPlacementClear(snapshot, cellX, cellY) {
  ...
  const x = cellX - snapshot.staticMap.cellX;
  const y = cellY - snapshot.staticMap.cellY;
  if (x < 0 || y < 0 || x >= snapshot.staticMap.width || y >= snapshot.staticMap.height) {
    return false;
  }
  const flags = snapshot.placement.flags[y * snapshot.staticMap.width + x];
  return Boolean(flags & 2);
}

function eastBSamSpinePathSample(snapshot) {
  const cells = [
    { cellX: 13, cellY: 21 }, { cellX: 13, cellY: 22 }, { cellX: 13, cellY: 23 },
    { cellX: 13, cellY: 24 }, { cellX: 12, cellY: 20 }, { cellX: 12, cellY: 21 },
    { cellX: 11, cellY: 22 }, { cellX: 14, cellY: 22 }, { cellX: 13, cellY: 20 },
  ];
  return cells.map((cell) => ({
    ...cell,
    clear: eastBSamPlacementClear(snapshot, cell.cellX, cell.cellY),
  }));
}

function eastBSamSpinePathBlocked(snapshot) {
  if (!snapshot?.placement?.flags) return false;
  return !eastBSamPlacementClear(snapshot, 13, 22)
    || !eastBSamPlacementClear(snapshot, 13, 23);
}

function eastBSamCellOccupied(snapshot, cellX, cellY, ignoreKey = undefined) {
  return snapshot?.objects?.some((object) => (
    object.cellX === cellX && object.cellY === cellY && object.strength > 0
    && (!ignoreKey || objectKey(object) !== ignoreKey)
  )) ?? false;
}

function eastBSamKillCellClear(snapshot, cellX, cellY, ignoreKey = undefined) {
  return eastBSamPlacementClear(snapshot, cellX, cellY)
    && !eastBSamCellOccupied(snapshot, cellX, cellY, ignoreKey);
}

function eastBSamPickClearKillCell(snapshot, candidates, ignoreKey = undefined) {
  for (const candidate of candidates) {
    if (eastBSamKillCellClear(snapshot, candidate.cellX, candidate.cellY, ignoreKey)) {
      return candidate;
    }
  }
  return undefined;
...
function queueEastBSamKillLineRole(commands, role, tank, snapshot, candidates, westernSam,
  forceMove = false) {
  const target = eastBSamPickClearKillCell(snapshot, candidates, objectKey(tank));
  const flags = forceMove ? MODIFIER_ALT : 0;
  if (target) {
    queueMissionEightRole(commands, role, [tank], target, flags, 1);
    return;
  }
  queueMissionEightRole(commands, `${role}-sam`, [tank], westernSam, flags, 1);
}

function eastBSamKillWindowOccupancy(snapshot) {
  const cells = [
    { cellX: 14, cellY: 20 }, { cellX: 14, cellY: 21 }, { cellX: 14, cellY: 22 },
    { cellX: 13, cellY: 20 }, { cellX: 13, cellY: 21 }, { cellX: 13, cellY: 22 },
    { cellX: 13, cellY: 23 }, { cellX: 13, cellY: 24 },
    { cellX: 12, cellY: 20 }, { cellX: 12, cellY: 21 }, { cellX: 11, cellY: 20 },
  ];
  return cells.map((cell) => ({
    ...cell,
    terrainClear: eastBSamPlacementClear(snapshot, cell.cellX, cell.cellY),
    occupants: snapshot.objects.filter((object) => (
      object.cellX === cell.cellX && object.cellY === cell.cellY && object.strength > 0
    )).map(({ typeName, id, owner, strength }) => ({ typeName, id, owner, strength })),
  }));
}

function eastBSamRecordKillOccupancy(snapshot, westernSam) {
  if (!westernSam || westernSam.strength <= 0
    || westernSam.strength >= westernSam.maxStrength
    || westernSam.strength > 220) return;
  if (snapshot.tick < 32400 || snapshot.tick > 32700) return;
  missionEightState.eastBSamKillOccupancy = eastBSamKillWindowOccupancy(snapshot);
}

function eastBSamLogSpinePathSample(snapshot, westernSam) {
  const state = missionEightState;
  if (state.eastBSamSpinePathLogged || !westernSam
    || westernSam.strength >= westernSam.maxStrength) return;
  state.eastBSamSpinePathSample = eastBSamSpinePathSample(snapshot);
  state.eastBSamSpinePathLogged = true;
  if (process.env.CNCWEB_VERIFY_TRACE) {
    console.error(JSON.stringify({
      eastBSamSpinePath: state.eastBSamSpinePathSample,
      tick: snapshot.tick,
      samStrength: westernSam.strength,
    }));
  }
}
...
function eastBSamWestPreStep(tank, samDist) {
  ...
  if (samDist === 7 && tank.cellX <= 12 && tank.cellY >= 23) {
    return { cellX: 11, cellY: 22 };
  }
  if (samDist === 6 && tank.cellX <= 11 && tank.cellY === 22) {
    return eastBSamKillFireCell;
  }
...
function eastBWestPrePositionPick(tanks) {
...
function eastBSamPartnerWestStep(tank, snapshot, westernSam, liveTanks) {
  ...
  if (tank.cellX === 13 && tank.cellY >= 22) {
    const sameY = { cellX: 12, cellY: tank.cellY };
    if (eastBSamKillCellClear(snapshot, sameY.cellX, sameY.cellY, tankKey)) {
      return { fire: false, target: sameY };
    }
    const northWest = { cellX: 12, cellY: Math.max(tank.cellY - 1, 21) };
    if (eastBSamKillCellClear(snapshot, northWest.cellX, northWest.cellY, tankKey)) {
      return { fire: false, target: northWest };
    }
    return { fire: false, target: sameY };
  }
  if (tank.cellX === 12 && tank.cellY > 21) {
    const north = { cellX: 12, cellY: tank.cellY - 1 };
    if (eastBSamKillCellClear(snapshot, north.cellX, north.cellY, tankKey)) {
      return { fire: false, target: north };
    }
  }
  if (tank.cellX === 12 && tank.cellY === 21 && samDist > 5) {
    const flank = eastBSamSecondShooterCell(liveTanks);
    if (eastBSamKillCellClear(snapshot, flank.cellX, flank.cellY, tankKey)) {
      return { fire: false, target: flank };
    }
  }
  if (tank.cellX === 12 && tank.cellY >= 22 && samDist > 5
    && westernSam.strength <= 186) {
    return { fire: false, target: eastBSamSecondShooterCell(liveTanks) };
  }
  if (tank.cellX === 11 && tank.cellY === 22 && samDist > 5) {
    return { fire: false, target: eastBSamSecondShooterCell(liveTanks) };
  }
  return { fire: false, target: eastBSamSecondShooterCell(liveTanks) };
}
...
function queueEastBSamPartnerWestRoute(commands, snapshot, westernSam, liveTanks,
  ...
  const step = eastBSamPartnerWestStep(partner, snapshot, westernSam, liveTanks);
...
function eastBSamEastColumnDetourEligible(tank, spineFinisher = undefined) {
  if (spineFinisher && objectKey(tank) === objectKey(spineFinisher)) return false;
  if (tank.cellX >= 15 && tank.cellY >= 20 && tank.cellY <= 26) return true;
  if (tank.cellX === 14 && tank.cellY >= 24 && tank.cellY <= 26) return true;
  return false;
}
...
function eastBSamEastKillCorridorTank(tank, spineFinisher = undefined) {
  // East-of-spine positions always use the detour path — even for the designated
  // spine finisher (TRACE v270: #29 @15,23 lost corridor skip → dist-7 pull @14,22).
  if (tank.cellX >= 15 && tank.cellY >= 20 && tank.cellY <= 26) return true;
  if (tank.cellX === 14 && tank.cellY >= 24 && tank.cellY <= 26) return true;
  if (tank.cellX === 13 && tank.cellY >= 22 && tank.cellY <= 26) return true;
  if (spineFinisher && objectKey(tank) === objectKey(spineFinisher)) return false;
  return false;
}
...
function eastBSamEastSpineDetourStep(tank, snapshot = undefined, samStrength = undefined) {
  ...
  if (tank.cellX === 14 && tank.cellY >= 20 && tank.cellY < 24) {
    return { cellX: 14, cellY: tank.cellY + 1 };
  }
  if (tank.cellX === 14 && tank.cellY >= 24) {
    return { cellX: 13, cellY: tank.cellY };
  }
  if (tank.cellX === 13 && tank.cellY > 21) {
    if (snapshot && eastBSamCellOccupied(snapshot, 13, tank.cellY - 1, tankKey)) {
      return { cellX: 12, cellY: Math.max(tank.cellY - 1, 20) };
    }
    return { cellX: 13, cellY: tank.cellY - 1 };
  }
  return eastBSamKillSpineStage;
}
...
function queueEastBSamSpineBlockerYield(commands, liveTanks, samStrength, spineFinisher = undefined) {
  ...
  const onSpine = liveTanks.filter((tank) => (
    tank.typeName === "MTNK" && tank.strength > 0
    && tank.cellX === 13 && tank.cellY >= 23 && tank.cellY <= 27
  )).toSorted((left, right) => left.cellY - right.cellY || left.id - right.id);
  for (let index = 0; index < onSpine.length - 1; index += 1) {
...
function eastBSamSecondShooterCell(liveTanks) {
  const westLead = eastBWestPrePositionPick(liveTanks);
  if (westLead && westLead.cellX === 11 && westLead.cellY === 20) {
    return { cellX: 11, cellY: 21 };
  }
  return eastBSamKillNorthFlankCell;
}

function eastBSamSpineKillShooterPick(liveTanks, spineFinisher = undefined, westernSam = undefined) {
  const samFocus = westernSam ?? { cellX: 13, cellY: 16 };
  return liveTanks.filter((tank) => (
    tank.typeName === "MTNK" && tank.strength > 0
    && eastBSamEastKillCorridorTank(tank, spineFinisher)
    && tank.cellX === 13 && tank.cellY >= 22 && tank.cellY <= 26
  )).toSorted((left, right) => (
    left.cellY - right.cellY || left.id - right.id
  ))[0];
}

function eastBSamSpineSecondShooterStep(tank, snapshot, westernSam, samStrength = 999, liveTanks = []) {
  const tankKey = objectKey(tank);
  const samDist = missionEightDistance(tank, westernSam);
  const secondCell = eastBSamSecondShooterCell(liveTanks);
  if (samDist <= 5) return { fire: true, target: westernSam };
  if (tank.cellX === 13 && tank.cellY > 21 && samStrength <= 220) {
    const north = { cellX: 13, cellY: tank.cellY - 1 };
    if (!snapshot || eastBSamKillCellClear(snapshot, north.cellX, north.cellY, tankKey)) {
      return { fire: false, target: north };
    }
    return { fire: false, target: { cellX: 12, cellY: tank.cellY } };
  }
  if (tank.cellX === 12 && tank.cellY >= 22 && samStrength <= 220 && samDist > 5) {
    return { fire: false, target: secondCell };
  }
  return {
    fire: false,
    target: eastBSamEastSpineDetourStep(tank, snapshot),
  };
}

function queueEastBSamSpineShooterYield(commands, liveTanks, spineFinisher, spineShooter, samStrength) {
  if (samStrength > 220 || !spineShooter) return;
  for (const tank of liveTanks) {
    if (tank.typeName !== "MTNK" || tank.strength <= 0) continue;
    if (objectKey(tank) === objectKey(spineShooter)) continue;
    if (tank.cellX !== 13 || tank.cellY < 22 || tank.cellY > 27) continue;
    if (!eastBSamEastKillCorridorTank(tank, spineFinisher)) continue;
    const yieldY = tank.cellY >= spineShooter.cellY
      ? Math.min(tank.cellY + 1, 27)
      : tank.cellY;
    queueMissionEightRole(commands, `east-b-sam-spine-shooter-yield-${objectKey(tank)}`,
      [tank], { cellX: 12, cellY: yieldY }, MODIFIER_ALT, 1);
  }
}

function queueEastBSamWestEdgeKill(commands, snapshot, westernSam, liveTanks, maxSamStrength = 220,
  approachOnly = false, spineFinisher = undefined) {
  const killLineKeys = new Set();
  const samStrength = westernSam.strength;
  if (samStrength <= 0 || samStrength > maxSamStrength) return killLineKeys;
  const chipBandLead = maxSamStrength >= 280;
  const eastCloseCandidates = [
    { cellX: 14, cellY: 20 },
    { cellX: 14, cellY: 21 },
    { cellX: 15, cellY: 21 },
    { cellX: 14, cellY: 22 },
  ];
  const westCloseCandidates = [
    eastBSamKillFireCell,
    eastBSamKillAltFireCell,
    { cellX: 11, cellY: 21 },
    { cellX: 12, cellY: 20 },
  ...
  for (const tank of liveTanks) {
    ...
    const onEastKillLine = tank.cellX >= 14 && tank.cellX <= 15
      && tank.cellY >= 20 && tank.cellY <= 22;
    if (samDist <= 5 && (onWestKillLine || onEastKillLine)) {
      ...
      killLineKeys.add(tankKey);
      queueEastBSamKillLineRole(commands, `east-b-sam-kill-line-${tankKey}`,
        tank, snapshot, [{ cellX: 14, cellY: 23 }, { cellX: 14, cellY: 22 }], westernSam);
    }
  }
  return killLineKeys;
}

function eastBSamSpineCorridorYieldNeeded(liveTanks, spineFinisher) {
  if (!spineFinisher || spineFinisher.cellX !== 13
    || spineFinisher.cellY < 22 || spineFinisher.cellY > 25) return undefined;
  return liveTanks.find((tank) => (
    tank.typeName === "MTNK" && tank.strength > 0
    && objectKey(tank) !== objectKey(spineFinisher)
    && eastBSamEastKillCorridorTank(tank, spineFinisher)
    && (
      (tank.cellX === 13 && tank.cellY > spineFinisher.cellY && tank.cellY <= 26)
      || (tank.cellX >= 15 && tank.cellY >= 24 && tank.cellY <= 26)
      || (tank.cellX === 14 && tank.cellY >= 24 && tank.cellY <= 26)
    )
  ));
}

function queueEastBSamSpineCorridorYield(commands, snapshot, westernSam, spineFinisher, liveTanks) {
  const yieldKeys = new Set();
  if (!spineFinisher || spineFinisher.strength <= 0) return yieldKeys;
  const samStrength = westernSam.strength;
  if (samStrength <= 0 || samStrength > 220) return yieldKeys;
  if (!eastBSamSpineCorridorYieldNeeded(liveTanks, spineFinisher)) return yieldKeys;
  const key = objectKey(spineFinisher);
  yieldKeys.add(key);
  const flank = spineFinisher.cellY > 22
    ? { cellX: 12, cellY: 21 }
  ...
  const target = eastBSamPickClearKillCell(snapshot, candidates, key) ?? flank;
  queueMissionEightRole(commands, `east-b-sam-spine-yield-${key}`,
    [spineFinisher], target, MODIFIER_ALT, 1);
  return yieldKeys;
}

function queueEastBSamSpineKillClose(commands, snapshot, westernSam, spineFinisher) {
  const spineKeys = new Set();
  if (!spineFinisher || spineFinisher.strength <= 0) return spineKeys;
  const samStrength = westernSam.strength;
  if (samStrength <= 0 || samStrength > 220) return spineKeys;
  ...
  if (samStrength <= 220 && samDist > 5 && spineFinisher.cellX === 13 && spineFinisher.cellY > 21) {
    ...
    const westFlankFire = samStrength <= 200 && spineFinisher.cellY >= 23 && samDist >= 7;
    ...
    if (westFlankFire) {
      const westTarget = eastBSamPickClearKillCell(snapshot, [
        { cellX: 12, cellY: 21 },
        { cellX: 12, cellY: 22 },
        eastBSamKillNorthFlankCell,
        { cellX: 12, cellY: spineFinisher.cellY },
      ...
      if (westTarget) {
        queueMissionEightRole(commands, `east-b-sam-spine-west-flank-${key}`,
          [spineFinisher], westTarget, MODIFIER_ALT, 1);
        return spineKeys;
      }
    }
  ...
  if (samStrength <= 220 && samDist > 5 && spineFinisher.cellX === 12 && spineFinisher.cellY >= 22) {
    ...
    if (samStrength <= 200 && samDist >= 7 && spineFinisher.cellY >= 23) {
      ...
      if (holdWest) {
        queueMissionEightRole(commands, `east-b-sam-spine-west-hold-${key}`,
          [spineFinisher], holdWest, MODIFIER_ALT, 1);
        return spineKeys;
      }
    }
  ...
  if (spineFinisher.cellY >= 24) {
    const northCandidates = spineFinisher.cellX === 13
      ...
      : [
        { cellX: 12, cellY: spineFinisher.cellY - 1 },
        { cellX: 12, cellY: spineFinisher.cellY - 2 },
        { cellX: 13, cellY: spineFinisher.cellY - 1 },
        eastBSamKillSpineStage,
        eastBSamKillNorthFlankCell,
      ];
    target = eastBSamPickClearKillCell(snapshot, northCandidates, key);
    if (!target) {
      queueMissionEightRole(commands, `east-b-sam-spine-fire-${key}`,
        [spineFinisher], westernSam, 0, 1);
      return spineKeys;
    ...
    target = eastBSamSpineFinisherRailStep(spineFinisher, samStrength, snapshot);
  }
  const rush = samStrength <= 220
    || (samStrength <= 240 && spineFinisher.cellY >= 26)
    || (samStrength <= 186 && spineFinisher.cellY >= 22);
  const spineForce = spineFinisher.cellY >= 24 && samStrength <= 220;
  queueMissionEightRole(commands, `east-b-sam-spine-close-${key}`,
    [spineFinisher], target, spineForce ? MODIFIER_ALT : (rush ? 0 : MODIFIER_ALT), 1);
  return spineKeys;
}

function eastBSpineFinisherPick(tanks, westernSam) {
  return tanks.filter((tank) => {
    if (tank.typeName !== "MTNK" || tank.strength < 50) return false;
    const produced = missionEightState.eastBProducedTankKeys.has(objectKey(tank));
    if (!produced && tank.strength < Math.ceil(tank.maxStrength * 0.85)) return false;
    return tank.cellX >= 9 && tank.cellX <= 16
      && (tank.cellY >= 23 || (produced && tank.cellY >= 28))
      && missionEightDistance(tank, westernSam) > 5;
  }).toSorted((left, right) => (
    Number(missionEightState.eastBProducedTankKeys.has(objectKey(right)))
      - Number(missionEightState.eastBProducedTankKeys.has(objectKey(left)))
    || right.strength - left.strength
    || right.cellY - left.cellY
    || left.cellX - right.cellX
    || left.id - right.id
...
function eastBSpineFinisherResolve(tanks, westernSam) {
  const state = missionEightState;
  if (state.eastBSamSpineFinisherKey) {
    const designated = tanks.find((tank) => (
      objectKey(tank) === state.eastBSamSpineFinisherKey && tank.strength > 0
    ));
    if (designated && missionEightDistance(designated, westernSam) > 5) return designated;
  }
  return eastBSpineFinisherPick(tanks, westernSam);
}

function eastBSamSpineFinisherRailStep(tank, samStrength = 999, snapshot = undefined) {
  if (tank.cellX >= 15 && tank.cellY >= 22 && tank.cellY <= 26) {
    return eastBSamEastSpineDetourStep(tank, snapshot, samStrength);
  }
  const tankKey = objectKey(tank);
  const westDetour = snapshot && eastBSamSpinePathBlocked(snapshot);
  if (westDetour && tank.cellY >= 22) {
    if (tank.cellY <= 21 && tank.cellX <= 12) return eastBSamKillNorthFlankCell;
    if (tank.cellX >= 13) return { cellX: 12, cellY: Math.max(tank.cellY - 1, 21) };
    return { cellX: 12, cellY: Math.max(tank.cellY - 1, 20) };
  }
  if (snapshot && samStrength <= 200 && tank.cellX === 13 && tank.cellY >= 23
    && (eastBSamCellOccupied(snapshot, 13, 22, tankKey)
      || eastBSamCellOccupied(snapshot, 13, 23, tankKey)
      || missionEightDistance(tank, { cellX: 13, cellY: 16 }) >= 7)) {
    const westStep = eastBSamPickClearKillCell(snapshot, [
      { cellX: 12, cellY: Math.max(tank.cellY - 1, 21) },
      { cellX: 12, cellY: tank.cellY },
      { cellX: 12, cellY: 21 },
    ], tankKey);
    if (westStep) return westStep;
    return { cellX: 12, cellY: Math.max(tank.cellY - 1, 21) };
  }
  if (samStrength <= 220 && tank.cellX === 13 && tank.cellY >= 22) {
    return tank.cellY <= 21 ? eastBSamKillNorthFlankCell : eastBSamKillSpineStage;
  }
  ...
  if (tank.cellX !== 13 && tank.cellY >= 24 && tank.cellY <= 34) {
    return { cellX: 13, cellY: Math.min(tank.cellY, 28) };
  }
  if (samStrength <= 186 && tank.cellX === 13 && tank.cellY >= 24) {
    return eastBSamKillSpineStage;
  }
  if (samStrength <= 186 && tank.cellX >= 12 && tank.cellX <= 14 && tank.cellY >= 22) {
    return eastBSamKillNorthFlankCell;
  }
  if (samStrength <= 200 && tank.cellX === 13 && tank.cellY >= 24 && tank.cellY <= 28) {
    return eastBSamKillWestStage;
  }
  if (tank.cellY >= 40 || tank.cellX >= 16 || tank.cellX <= 8) return { cellX: 13, cellY: 32 };
  if (tank.cellY >= 28) return { cellX: 13, cellY: 24 };
  if (tank.cellY >= 23) return eastBSamKillWestStage;
  if (tank.cellX <= 11 && tank.cellY === 22) return eastBSamKillFireCell;
  if (tank.cellY >= 21) return eastBSamKillNorthFlankCell;
  return eastBSamKillFireCell;
}

function queueEastBSamSpineFinisherRail(commands, snapshot, strike, westernSam, liveTanks) {
  if (!westernSam || westernSam.strength <= 0
    || westernSam.strength >= westernSam.maxStrength) return false;
  eastBSamLogSpinePathSample(snapshot, westernSam);
  ...
  if (samStrength > 285) return false;
  const target = eastBSamSpineFinisherRailStep(spineFinisher, samStrength, snapshot);
  const rush = samStrength <= 220
    || (samStrength <= 240 && spineFinisher.cellY >= 26)
    || (samStrength <= 186 && spineFinisher.cellY >= 22);
  queueMissionEightRole(commands, `east-b-spine-rail-${objectKey(spineFinisher)}`,
    [spineFinisher], target, rush ? 0 : MODIFIER_ALT, 1);
  return true;
}
...
function eastBSamDeepSpineFinisherPick(liveTanks, westernSam) {
  ...
  const produced = liveTanks.filter((tank) => (
    ...
    right.strength - left.strength || left.cellY - right.cellY || left.id - right.id
  ))[0];
  return produced ?? eastBSpineFinisherResolve(liveTanks, westernSam);
}

function queueEastBSamDeepFinisher(commands, snapshot, strike, westernSam) {
  ...
  const killLineKeys = queueEastBSamWestEdgeKill(commands, snapshot, westernSam, liveTanks, 220,
    false, spineFinisherTank);
  const spineKillKeys = spineFinisherTank
    ? queueEastBSamSpineKillClose(commands, snapshot, westernSam, spineFinisherTank)
    : new Set();
  for (const tank of liveTanks) {
    const samDist = missionEightDistance(tank, westernSam);
    const tankKey = objectKey(tank);
    ...
    if (samStrength <= 220 && eastBSamEastKillCorridorTank(tank, spineFinisherTank)
      ...
      if (spineFinisherTank && objectKey(tank) !== objectKey(spineFinisherTank)
        ...
        && tank.cellX >= 12 && tank.cellY >= 22) {
        const step = eastBSamPartnerWestStep(tank, snapshot, westernSam, liveTanks);
        queueMissionEightRole(commands, step.fire
      ...
      if (!spineFinisherTank && spineShooter && objectKey(tank) === objectKey(spineShooter)
        && ((tank.cellX === 13 && tank.cellY >= 22)
          || (tank.cellX === 12 && tank.cellY >= 22))) {
        const shooterStep = eastBSamSpineSecondShooterStep(tank, snapshot, westernSam, samStrength, liveTanks);
        queueMissionEightRole(commands, shooterStep.fire
          ? `east-b-sam-deep-spine-fire-${tankKey}`
          : `east-b-sam-deep-spine-north-${tankKey}`,
          [tank], shooterStep.target, shooterStep.fire ? 0 : MODIFIER_ALT, 1);
        continue;
      }
      if (samDist <= 5) {
        queueMissionEightRole(commands, `east-b-sam-deep-corridor-fire-${tankKey}`,
          [tank], westernSam, 0, 1);
        continue;
      }
      queueMissionEightRole(commands, `east-b-sam-deep-spine-${tankKey}`,
        [tank], eastBSamEastSpineDetourStep(tank, snapshot, samStrength),
        eastBSamEastSpineDetourModifier(tank, samStrength), 1);
      continue;
    }
    ...
    if (samDist <= 5) {
      ...
      continue;
    } else if (samStrength <= 180 && tank.cellX === 14 && tank.cellY >= 24) {
      queueMissionEightRole(commands, `east-b-sam-deep-east-rush-${objectKey(tank)}`,
        [tank], eastBSamWestCloseTarget(tank), MODIFIER_ALT, 1);
    } else if (samStrength <= 186 && tank.strength >= 120
      && tank.cellY >= 24 && tank.cellX <= 14
      && !(spineFinisherTank && objectKey(tank) === objectKey(spineFinisherTank))) {
      const backupStep = tank.cellY > 22
        ? { cellX: 13, cellY: 21 }
        : eastBSamKillNorthFlankCell;
      queueMissionEightRole(commands, `east-b-sam-deep-backup-${objectKey(tank)}`,
        [tank], backupStep, MODIFIER_ALT, 1);
    } else {
  ...
  const westShooterReady = liveTanks.some((tank) => (
    tank.cellX <= 12 && tank.cellY >= 20 && tank.cellY <= 22
    && missionEightDistance(tank, westernSam) <= 5
  ));
  if (deepRockets.length > 0) {
    ...
    if (samStrength <= 220 && (westShooterReady || soloWestShooter)) {
      ...
      queueMissionEightRole(commands, "east-b-sam-deep-rocket-hold", deepRockets,
        { cellX: 13, cellY: 30 }, MODIFIER_ALT, 1);
    } else {
      queueMissionEightRole(commands, "east-b-sam-deep-rocket-hold", deepRockets,
  ...
  if (spineKillKeys.size === 0) {
    queueEastBSamSpineFinisherRail(commands, snapshot, strike, westernSam, liveTanks);
  }
}
...
function queueMissionEightForces(snapshot, friendly, hostiles, attackers, commands) {
  ...
  if (mission.variant === "east-a" && state.engineer.captureTick !== undefined) {
    ...
    if (state.postFactCleanupLaunchTick === undefined
      ...
      if (eastAEarlyCapture(state)) {
        state.postFactCleanupTransitStage =
          missionEightEastAPostFactCleanupTransitRoute.length;
        eastAPrepareEarlyCaptureCleanup(state, hostiles, snapshot);
      }
  ...
  if (mission.variant === "east-b") {
    ...
    if (postSamDefense) {
      ...
      // Post-western-SAM east-b: tighter WEAP picket — TRACE v391k lost WEAP while
      // base guards wandered on a loose 16-cell threat radius.
      const eastBPostWestBase = mission.variant === "east-b" && eastBWesternSamDead(hostiles)
        && state.allSamsDeadTick === undefined;
      const weapLive = mission.variant === "east-b"
        ? friendly.find((object) => (
          object.type === 4 && object.typeName === "WEAP" && object.strength > 0
        ))
        : undefined;
      const defensePoint = eastBPostWestBase && weapLive
        ? { cellX: weapLive.cellX + 1, cellY: weapLive.cellY + 1 }
        : basePoint;
      // TRACE baseline: radius 10 chase killed all pad armor by 35.4k. Hold at 5
      // so the pinned WEAP defender survives until free emerges.
      const baseThreat = missionEightThreatNear(hostiles, defensePoint,
        mission.variant === "east-a" ? 10 : (eastBPostWestBase ? 5 : 16));
      const baseMtnk = baseGuard.filter((guard) => guard.typeName === "MTNK");
      const baseOther = baseGuard.filter((guard) => guard.typeName !== "MTNK");
      if (eastBPostWestBase && baseMtnk.length > 0) {
        // Pinned defender first: only engage threats within 5 of WEAP.
        const pinned = state.eastBPostWestWeapDefenderKey
          ? baseMtnk.filter((g) => objectKey(g) === state.eastBPostWestWeapDefenderKey)
          : [];
        const otherMtnk = baseMtnk.filter((g) => (
          objectKey(g) !== state.eastBPostWestWeapDefenderKey
        ));
        if (pinned.length > 0) {
          queueMissionEightRole(commands, "east-b-weap-defender", pinned,
            baseThreat ?? defensePoint, baseThreat ? 0 : MODIFIER_ALT, 15);
        }
        if (otherMtnk.length > 0) {
          queueMissionEightRole(commands, "east-b-weap-picket", otherMtnk,
            baseThreat ?? defensePoint, baseThreat ? 0 : MODIFIER_ALT, 20);
        }
        if (baseOther.length > 0) {
          queueMissionEightRole(commands, "east-b-base-screen", baseOther,
            baseThreat ?? defensePoint, baseThreat ? 0 : MODIFIER_ALT, 45);
        }
      } else {
        queueMissionEightRole(commands, "base", baseGuard, baseThreat ?? defensePoint, 0, 60);
      }
    }
  }

  if (mission.variant === "east-b") {
    // l513: light Moebius restaff only (y≥55 x≤18) when residual — keep vg
    // alive past pass2@47k without mass mid-map re-home desync (l504).
    if (state.eastBResidualCommit || state.allSamsDeadTick !== undefined) {
      for (const unit of attackers) {
        if (unit.type !== 1 || unit.strength <= 0) continue;
        if (unit.cellY < 55 || unit.cellX > 18) continue;
        const key = objectKey(unit);
        state.villageGuardKeys.add(key);
        state.strikeKeys.delete(key);
        state.baseGuardKeys.delete(key);
    ...
    // Post-west: do not refill village from free tanks while remaining SAMs
    // live and combat force is thin (TRACE v395b free→village, NW SAM 400).
    const postWestSamPush = eastBWesternSamDead(hostiles)
      && state.allSamsDeadTick === undefined;
    const combatMtnkCount = attackers.filter((attacker) => (
      attacker.typeName === "MTNK" && attacker.strength >= 40
      && (state.strikeKeys.has(objectKey(attacker))
        || (state.eastBProducedTankKeys.has(objectKey(attacker))
          && !state.villageGuardKeys.has(objectKey(attacker))
          && !state.baseGuardKeys.has(objectKey(attacker))))
    ...
    if (liveVillageTanks < villageTankTarget && !protectAssaultStaging
      ...
      && !(postWestSamPush && combatMtnkCount < 2)) {
      ...
      for (const tank of candidates) {
        ...
        // Post-west surplus only: keep ≥1 free/strike for SAM.
        if (postWestSamPush && combatMtnkCount - (liveVillageTanks > 0 ? 0 : 0) < 2) {
          break;
        }
        const key = objectKey(tank);
        clearMissionEightUnitRoleKey(key);
    ...
    // Post-western-SAM: widen y to 30 so mid-map raiders after the SAM kill are
    ...
    // Post-western-SAM (v391): re-open base loan when village is thin — TRACE
    // v390 lost civs 34.5k–38k with villageGuard empty after the SAM kill.
    // Keep the early-window 0.5 HP gate intact (v391 0.35 desynced free-tank
    // timing and lost the western SAM kill).
    const westernSamDeadVillage = eastBWesternSamDead(hostiles);
    const postWestVillageLoan = westernSamDeadVillage
      && state.allSamsDeadTick === undefined
      && liveVillageTanks < missionEightEastBVillageTankCountLate;
    ...
    } else if (postWestVillageLoan || (westernSamDeadVillage
      && (armorThreat || infantryThreat))) {
      // Never loan the sole strike tank back to village (v395c SAM push).
      const strikeMtnkLive = attackers.filter((attacker) => (
        attacker.typeName === "MTNK" && state.strikeKeys.has(objectKey(attacker))
        && attacker.strength > 0
      )).length;
      const baseLoan = attackers.filter((attacker) => {
        if (attacker.typeName !== "MTNK") return false;
        if (attacker.strength < Math.ceil(attacker.maxStrength * 0.35)) return false;
        const key = objectKey(attacker);
        if (state.baseGuardKeys.has(key)) return true;
        // Southern strike tank may help civs only when we have 2+ strike MTNKs.
        if (postWestVillageLoan && state.strikeKeys.has(key)
          && attacker.cellY >= 36 && strikeMtnkLive >= 2) {
          return true;
        }
        return false;
      }).toSorted((left, right) => (
        right.strength / right.maxStrength - left.strength / left.maxStrength
        || left.id - right.id
      )).slice(0, postWestVillageLoan && strikeMtnkLive >= 2 ? 2 : 1);
      picket(
        baseLoan,
        hunterThreat ?? anchorThreat,
        airliftActive ? airliftRally : screenRally,
        "east-b-base-loan-post-west",
        10,
      );
    }
    ...
    // l509: free holds pad@145 through 46k; lose civ-near@46711 ~400t before
    // A-10 pass2 rearm@~47100. Moebius@19 nearly los3. ≥4 pickets on Moebius
    // from residual; engage d≤14 TRAN/E4 first. Hold until multi-pass A-10.
    const moebiusPicketCell = { cellX: 6, cellY: 58 };
    const hospPicketCell = { cellX: 4, cellY: 59 };
    ...
    const moebiusPickets = postAllSamVillage
      ? sortedInf.slice(0, Math.min(4, sortedInf.length))
      : [];
    const freeVillageInf = postAllSamVillage
      ? sortedInf.slice(moebiusPickets.length)
      : villageInfantry;
    if (moebiusPickets.length > 0) {
      // l510 TRACE freeLast@50100 free@158: engage d≤14 with half body on Moebius.
      // l511 d≤6-only regressed freeLast@41400 (Moebius dead). Restore l510.
      const moeThreat = hostiles.filter((h) => (
        h.strength > 0
        && missionEightDistance(moebiusPicketCell, h) <= 14
        && (h.typeName === "E1" || h.typeName === "E3" || h.typeName === "E4"
          || h.typeName === "BGGY" || h.typeName === "LTNK" || h.typeName === "TRAN")
      )).toSorted((a, b) => (
        Number(a.typeName !== "TRAN") - Number(b.typeName !== "TRAN")
        || Number(a.typeName !== "E4") - Number(b.typeName !== "E4")
        || missionEightDistance(moebiusPicketCell, a) - missionEightDistance(moebiusPicketCell, b)
        || a.strength - b.strength
      ))[0];
      if (moeThreat) {
        const half = Math.max(1, Math.ceil(moebiusPickets.length / 2));
        queueMissionEightRole(commands, "east-b-moebius-picket-atk",
          moebiusPickets.slice(0, half), moeThreat, 0, 6);
        const holders = moebiusPickets.slice(half);
        if (holders.length > 0) {
          queueMissionEightRole(commands, "east-b-moebius-picket-body",
            holders, moebiusPicketCell, MODIFIER_ALT, 8);
        }
      } else {
        ...
        if (needMove.length > 0) {
          queueMissionEightRole(commands, "east-b-moebius-picket-hold", needMove,
            moebiusPicketCell, MODIFIER_ALT, 10);
        }
      }
    }
  ...
  if (mission.variant === "east-a" && state.postFactCleanupLaunchTick !== undefined) {
    const liveCleanup = attackers.filter((attacker) => (
      state.postFactCleanupCohortKeys.has(objectKey(attacker)) && attacker.strength > 0
    ));
    if (liveCleanup.length > 0) {
      for (const attacker of liveCleanup) {
        state.strikeKeys.add(objectKey(attacker));
      }
      strike = attackers.filter((attacker) => state.strikeKeys.has(objectKey(attacker)));
    }
  }
  if (mission.variant === "east-b" && state.eastBSamSpineFinisherHoldKeys.size > 0) {
    const westernSamHold = hostiles.find((hostile) => (
      hostile.typeName === "SAM" && hostile.cellX === 13 && hostile.cellY === 16
    ));
    const spineHoldCell = { cellX: 13, cellY: 32 };
    const releaseChipBand = Boolean(westernSamHold
      && westernSamHold.strength <= 280
      && westernSamHold.strength > 220
      && westernSamHold.strength < westernSamHold.maxStrength);
    for (const key of [...state.eastBSamSpineFinisherHoldKeys]) {
      const tank = attackers.find((attacker) => (
        objectKey(attacker) === key && attacker.typeName === "MTNK"
      ));
      if (!tank || tank.strength <= 0) {
        state.eastBSamSpineFinisherHoldKeys.delete(key);
        continue;
      }
      if (releaseChipBand) {
        state.eastBSamSpineFinisherHoldKeys.delete(key);
        ...
        state.villageGuardKeys.delete(key);
        state.eastBWaveTwoKeys.delete(key);
        const rally = tank.cellY >= 40 ? spineHoldCell : spineHoldCell;
        queueMissionEightRole(commands, `east-b-spine-hold-${key}`, [tank], rally, MODIFIER_ALT, 1);
      }
    }
    strike = attackers.filter((attacker) => state.strikeKeys.has(objectKey(attacker)));
  }
  // Advance past the western SAM stage even when the post-west rebuild hold
  // short-circuits strike orders (TRACE v391c: routeStage stuck at 7 after
  // west SAM death because post-west push returned before stage progress).
  if (mission.variant === "east-b" && state.samDeathTicks.has("13:16")
    && state.routeStage < missionEightRoutes["east-b"].length) {
    const westSamStage = missionEightRoutes["east-b"].findIndex((site) => (
      site.typeName === "SAM" && (site.targetCellX ?? site.cellX) === 13
      && (site.targetCellY ?? site.cellY) === 16
    ));
    if (westSamStage >= 0 && state.routeStage <= westSamStage) {
      while (state.routeStage <= westSamStage) {
        const cleared = missionEightRoutes["east-b"][state.routeStage];
        state.routeProgress.push({
          tick: snapshot.tick,
          stage: state.routeStage,
          label: cleared?.label ?? "western SAM cleared",
          cellX: cleared?.cellX ?? 13,
          cellY: cleared?.cellY ?? 16,
          arrivals: 0,
          reason: "western-sam-death",
        });
        state.routeStage += 1;
        state.routeStageStartedTick = snapshot.tick;
      }
  ...
  if (mission.variant === "east-b"
    && queueEastBSamPostWesternSamPush(commands, snapshot, hostiles, strike, attackers,
      friendly)) {
    return;
  }
  if (strike.length === 0
  ...
  if (mission.variant === "east-b" && (
    ...
    const waveTwo = strike.filter((attacker) => (
      (state.eastBWaveTwoKeys.has(objectKey(attacker))
        && !state.eastBSamSpineFinisherHoldKeys.has(objectKey(attacker)))
  ...
  const postFactCleanupTransitRoute = missionEightEastAPostFactCleanupTransitRoute;
  if (mission.variant === "east-a" && state.postFactCleanupLaunchTick !== undefined
    && !eastAEarlyCapture(state)
  ...
  if (state.routeStage < route.length) {
    ...
    const eastBWesternSamCleared = mission.variant === "east-b"
      ...
      && !routeTargetAlive;
    const eastBWesternGunCleared = mission.variant === "east-b"
      && waypoint.typeName === "GUN"
      && targetCellX === 11 && targetCellY === 18
      && !routeTargetAlive;
    if (!waitingForAssembly && !routeTargetAlive
      && (targetDestroyedOnStage || eastBWesternSamCleared || eastBWesternGunCleared
        || waypoint.typeName === "FACT"
  ...
  if (mission.variant === "east-b" && nextWaypoint.typeName
    ...
    if (samDeepChip && westernSam && westernSam.strength > 0) {
      queueEastBSamDeepFinisher(commands, snapshot, strike, westernSam);
      return;
    }
    const samChipBandEarly = Boolean(
      ...
      if (samCorridorEarly) {
        ...
        for (const tank of liveStrikeTanks) {
          ...
          queueMissionEightRole(commands, `east-b-sam-corridor-early-${objectKey(tank)}`,
            [tank], eastBSamEastSpineDetourStep(tank, snapshot, westernSam.strength),
        ...
        for (const tank of trailers) {
          ...
          if (samChipBandEarly && tank.cellX === 13 && tank.cellY >= 26) {
            queueMissionEightRole(commands, `east-b-sam-finish-spine-north-${objectKey(tank)}`,
              [tank], eastBSamKillSpineFireLine, MODIFIER_ALT, 1);
            continue;
          }
          const rally = samDeepChip
            ? { cellX: 13, cellY: 20 }
            : samChipBandEarly && tank.cellX === 13 && tank.cellY >= 24
              ? eastBSamKillSpineFireLine
              : tank.cellY >= 55
                ? { cellX: 13, cellY: 48 }
                : tank.cellY >= 40
                  ? { cellX: 13, cellY: 32 }
                  : tank.cellY >= 28
                    ? { cellX: 13, cellY: 24 }
                    : { cellX: 13, cellY: 20 };
          const finishRailCadence = samDeepChip || (samChipBandEarly && tank.cellX === 13)
            ? 1
            : samFinishRailCadence;
          queueMissionEightRole(commands, `east-b-sam-finish-rail-${objectKey(tank)}`,
            [tank], rally, MODIFIER_ALT, finishRailCadence);
        }
        if (front.length === 0) {
    ...
    const spineFinisherFocus = westernSam
      && westernSam.strength <= 280
      && westernSam.strength > 220
      && westernSam.strength < westernSam.maxStrength
      ? eastBSpineFinisherResolve(tanks, westernSam) : undefined;
    ...
    if (gunForming) {
      ...
      for (const tank of trailing) {
        if (spineFinisherFocus && objectKey(tank) === objectKey(spineFinisherFocus)) continue;
    ...
    if (!isGunFocus && focusIsUnit) {
      ...
      const westEdgeChipKeys = samChipBand && westernSam
        ...
        : new Set();
      if (spineFinisherChip) {
        queueEastBSamSpineFinisherRail(commands, snapshot, strike, westernSam, finishers);
      }
    ...
    if (samDeepChip && focusIsUnit && westernSam) {
      const killClose = liveStrikeTanks.filter((tank) => (
        ...
        && !(state.eastBSamSpineFinisherKey
          && objectKey(tank) === state.eastBSamSpineFinisherKey)
      ));
  ...
  if (mission.variant === "east-b" && state.assaultTick !== undefined) {
    ...
    const samEarlySpineRail = Boolean(
      westernSamLive && !missionEightState.eastBSamDeepChip
      && westernSamLive.strength > 220
      && westernSamLive.strength <= 280
      && westernSamLive.strength < westernSamLive.maxStrength
    );
    const spineFinisherWest = westernSamLive
      ? eastBSpineFinisherResolve(
        strike.filter((attacker) => attacker.typeName === "MTNK" && attacker.strength > 0),
        westernSamLive,
      ) : undefined;
    const spineFinisherRail = Boolean(
      spineFinisherWest && westernSamLive
      && westernSamLive.strength <= 285
      && westernSamLive.strength > 0
      && westernSamLive.strength < westernSamLive.maxStrength
      && missionEightDistance(spineFinisherWest, westernSamLive) > 5
    );
    ...
    if (needsRail.length > 0) {
      ...
      const westRailCadence = (samDeepFinishRail || samEarlySpineRail || spineFinisherRail) ? 1 : 10;
      for (const tank of needsRail) {
        const useSpineStep = spineFinisherRail
          && spineFinisherWest && objectKey(tank) === objectKey(spineFinisherWest);
        const rally = useSpineStep
      ...
      if (samFinishRail || spineFinisherRail
        || (!forceMoveCorridor && !nextWaypoint.typeName && front.length === 0)) {
        if (spineFinisherRail && westernSamLive) {
          queueEastBSamSpineFinisherRail(commands, snapshot, strike, westernSamLive);
        }
...
try {
  ...
  if (mission.number === 8) {
    initializeMissionEight(snapshot);
    if (trace && process.env.CNCWEB_VERIFY_TRACE_INITIAL !== "0") {
      console.error(JSON.stringify({
  ...
  while (!snapshot.terminal && snapshot.tick < MAX_TICKS) {
    ...
    if (mission.number === 8) {
      ...
      if (missionEightTraceActive(snapshot.tick)) {
        const missionEightPayload = traceCompact && mission.variant === "east-b"
          ? missionEightTraceCompactRecord(snapshot, attackers, hostiles)
          : {
        variant: mission.variant,
        tick: snapshot.tick,
        funds: snapshot.sidebar.credits + snapshot.sidebar.tiberium,
        ...
        console.error(JSON.stringify({ missionEight: missionEightPayload }));
      }
      if (commands.length > 0) {
        submitCommands(handle, snapshot.tick + 1, commands);
        commandBatches += 1;
  ...
  const eastBLoseDiagnosis = mission.number === 8 && mission.variant === "east-b" ? {
    loseHint: missionEightState.eastBLoseHint
      ?? (missionEightState.eastBNeutralDeaths.length >= 9 ? "civ-nine-neutral-unit-deaths"
        ...
        // l519cr: unknown@59k with free/Moebius/HOSP live + 7 tracked deaths is
        // lagged civ-nine (deaths in same 30t advance as GDILOSE before tracker).
        : (missionEightState.eastBNeutralDeaths.length >= 7
          && missionEightState.minimumNeutralUnits <= 8
          && missionEightState.eastBMoebiusMissingTick === undefined
          && missionEightState.eastBHospitalMissingTick === undefined)
          ? "likely-civ-nine-tracker-lag"
          : "unknown"),
  ...
  if (mission.number === 8) {
    ...
    if (finalHostiles !== 0) {
      console.error(JSON.stringify({
        eastBFinalHostilesDebug: remainingHostiles.map((h) => ({
          typeName: h.typeName, id: h.id, strength: h.strength,
          cellX: h.cellX, cellY: h.cellY, type: h.type, owner: h.owner,
        })),
        tick: finalSnapshot.tick,
        won: Boolean(gameOver.flags & 4),
        free: finalSnapshot.objects.filter((o) => o.typeName === "MTNK" && o.owner === 0 && o.strength > 0)
          .map((o) => ({ id: o.id, s: o.strength, x: o.cellX, y: o.cellY })),
        airOrders: state.airstrike.orders.slice(-6),
        stats: finalSnapshot.stats,
        minN: state.minimumNeutralUnits,
      }));
    }
    assert.equal(finalHostiles, 0,
    ...
    const eastAEarlyWin = mission.variant === "east-a"
      && state.engineer.captureTick !== undefined
      && state.postFactCleanupLaunchTick !== undefined;
    // East-b residual free path (l391): free peels village→east pad→west base
    // after all-SAM / A-10; does not re-walk the pre-assault authored sweep.
    const eastBResidualWin = mission.variant === "east-b"
      && state.allSamsDeadTick !== undefined
      && state.airstrike.orders.length >= 1
      && state.eastBResidualCommit;
    if (eastAEarlyWin) {
    ...
    } else if (eastBResidualWin) {
      assert.ok(state.routeStage >= 5,
        "GDI Mission 8 east-b residual win never advanced the western sweep route");
      assert.ok(state.airstrike.orders.some((order) => order.target === "AFLD"
        || order.target === "GUN" || order.target === "HAND" || order.target === "PROC"),
        "GDI Mission 8 east-b residual win never airstruck east targets");
    } else {
